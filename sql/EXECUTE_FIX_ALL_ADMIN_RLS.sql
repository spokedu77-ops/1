-- ================================================================
-- 🔐 Admin RLS 정책 완전 수정 - 실행 순서
-- 1. is_admin() 함수 수정 (name 포함)
-- 2. 모든 테이블의 RLS 정책 수정
-- ================================================================

SELECT '🔐 Admin RLS 정책 완전 수정 시작...' as status;

-- ================================================================
-- 1단계: is_admin() 함수 수정 (name 포함)
-- ================================================================
SELECT '📋 1단계: is_admin() 함수 수정...' as progress;
...
-- ================================================================
-- 9단계: chat 관련 테이블 정책 수정
-- ================================================================
SELECT '📋 9단계: chat 관련 테이블 정책 수정...' as progress;

-- chat_rooms
DROP POLICY IF EXISTS "chat_rooms_all" ON chat_rooms;
DROP POLICY IF EXISTS "chat_rooms_select_all" ON chat_rooms;
DROP POLICY IF EXISTS "chat_rooms_insert_all" ON chat_rooms;
DROP POLICY IF EXISTS "chat_rooms_update_all" ON chat_rooms;
DROP POLICY IF EXISTS "chat_rooms_delete_all" ON chat_rooms;
...
DROP POLICY IF EXISTS "chat_participants_select_all" ON chat_participants;
DROP POLICY IF EXISTS "chat_participants_insert_all" ON chat_participants;
DROP POLICY IF EXISTS "chat_participants_update_all" ON chat_participants;
DROP POLICY IF EXISTS "chat_participants_delete_all" ON chat_participants;
...
SELECT '✅ chat 관련 테이블 정책 수정 완료' as status;
...
FROM pg_policies 
WHERE tablename IN ('users', 'sessions', 'mileage_logs', 'session_count_logs', 'memos', 'todos', 'warmup_programs_composite', 'rotation_schedule', 'play_scenarios', 'chat_rooms', 'chat_messages', 'chat_participants')
GROUP BY tablename
ORDER BY tablename;

SELECT '🎉 전역 Admin RLS 정책 수정 완료!' as final_status;
SELECT '✅ is_admin() 함수가 users.is_admin, role, name 모두 확인' as result;
