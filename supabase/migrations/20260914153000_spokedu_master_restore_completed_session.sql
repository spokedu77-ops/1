-- Restore completed → scheduled. Keep attendance, activities, and memo.
-- Clear roster_locked_at so the session is not left scheduled with a lock.
...
begin
  ...
  if old.roster_locked_at is not null
    and new.roster_locked_at is distinct from old.roster_locked_at
    and not (old.status='completed' and new.status='scheduled' and new.roster_locked_at is null)
  then
...
begin
  ...
  if v_old.status <> p_status and not (
    ...
    or (v_old.status in ('cancelled','completed') and p_status='scheduled')
