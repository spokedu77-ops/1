-- Keep server materialization aligned with the Admin Note block transaction contract:
-- validate versions, apply field patches, soft-delete blocks, then create blocks.
-- This matches local op replay and prevents one transaction type from having
-- different meaning on the client and on the server.
