-- Allow Lite→Premium proration apply (source=upgrade) without loosening list-price checks
-- for initial, renewal, or generic webhook charges. Does not schedule or enable billing cron.
-- Upgrade order.amount may be a prorated difference; subscription.current_amount stays the
-- Premium list price so spm_subscriptions_current_amount_check and next renewals remain valid.
