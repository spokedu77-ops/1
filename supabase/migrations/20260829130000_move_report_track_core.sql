-- MOVE REPORT TRACK — core schema (Rev.2 / Scoring Manual v0.1)
-- Sprint: MOVE-TRACK-FOUNDATION-01
--
-- Pre-apply checklist (Rev.2):
--   1) observation_opportunity_band NULL → structured fields: API validation only (no DB CHECK)
--   2) support_level: categorical code comment (not ordinal score)
--   3) frw_seconds=1 Challenge excluded via mr_frw_growth_phase_eligible()
--   4) self_reengagement NULL vs false documented (SM-07, CASE H)
--   5) VIEWER PII: mr_children_impact_safe (child_code only, no child_name)
--   6) smoke: scripts/move-report-track-smoke.sql
...
CREATE TABLE public.mr_session_child_records (
  ...
  -- Absent: clear observational fields (attendance is hard invariant)
...
INSERT INTO public.mr_field_source_catalog (field_key, source_type, description) VALUES
  ...
  ('support_level', 'OBSERVED', 'SM-06 categorical primary support code, not ordinal performance score'),
  ('independent_initiation', 'OBSERVED', 'SM-04 initiation frequency NULL|0-3 — excludes direct action cues'),
  ('self_reengagement', 'OBSERVED', 'SM-07 NULL=no re-engagement observation opportunity; false=opportunity but no self return; true=self return without direct action prompt'),
  ('spomove_used', 'CONTEXT', 'Whether SPOMOVE was used'),
  ('frw_seconds', 'SYSTEM', 'SM-08 SPOMOVE setting 1-6 — sec 1 Challenge excluded from growth Impact'),
...
-- Growth/Impact: seconds 2-6 + observed_stable only; frw_seconds=1 Challenge excluded
...
-- PII-safe child projection for VIEWER / external Impact (no child_name, support_notes)
...
WHERE
  ...
  OR EXISTS (
    SELECT 1
    FROM public.mr_program_children pc
    JOIN public.mr_program_viewers pv ON pv.program_id = pc.program_id
    WHERE pc.child_id = c.id
      AND pv.user_id = (SELECT auth.uid())
      AND pv.impact_report_approved IS TRUE
  );
...
COMMENT ON VIEW public.mr_children_impact_safe IS
  'PII-safe child read path for VIEWER/Impact. Exposes child_code only — never child_name or support_notes. Instructors/admins use mr_children via API with role checks.';
...
-- mr_children: admin + instructor only — VIEWER must use mr_children_impact_safe
DROP POLICY IF EXISTS mr_children_select_member ON public.mr_children;
CREATE POLICY mr_children_select_instructor_admin
...
COMMENT ON TABLE public.mr_session_child_records IS
  'MOVE TRACK observation. NULL=Not Assessed, 0=assessed absent. observation_opportunity_band NULL rules enforced in API not DB. Typical Performance in structured fields; Meaningful Change in observation_note.';

COMMENT ON COLUMN public.mr_session_child_records.observation_opportunity_band IS
  'SM-02: one|two|three_plus; NULL=not assessed. When NULL, structured observational fields should be NULL — validated in API layer.';

COMMENT ON COLUMN public.mr_session_child_records.support_level IS
  'SM-06 Typical Performance: categorical primary support code, not ordinal performance score. NULL=Not Assessed; 0-4=category codes only.';

COMMENT ON COLUMN public.mr_session_child_records.independent_initiation IS
  'SM-04 OBSERVED frequency NULL|0-3. Excludes direct action cues. NULL=Not Assessed; 0=assessed, none observed.';

COMMENT ON COLUMN public.mr_session_child_records.self_reengagement IS
  'SM-07 tri-state: NULL=no re-engagement observation opportunity (e.g. continuous participation, CASE H); false=disengagement occurred but no self-initiated return without direct action prompt; true=self return without direct action prompt.';

COMMENT ON COLUMN public.mr_session_child_records.frw_seconds IS
  'SPOMOVE program setting 1-6 (SYSTEM). NOT reaction time. Value 1 = Challenge Setting — excluded from Growth/Impact via mr_frw_growth_phase_eligible().';

COMMENT ON COLUMN public.mr_session_child_records.frw_status IS
  'FRW stability. Growth/Impact uses observed_stable with frw_seconds 2-6 only.';

COMMENT ON COLUMN public.mr_children.child_code IS
  'External anonymous identifier for Impact (e.g. SPM-2027-001). VIEWER APIs must use this, never child_name.';

SELECT 'move_report_track_core applied.' AS status;
