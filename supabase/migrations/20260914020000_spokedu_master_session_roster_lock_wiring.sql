-- Roster lock wiring + completed→scheduled rejection.
-- Does not create participant tables, backfill completed rows, or reconnect dormant unused policies.
-- If spokedu_master_sessions_edit_policy is already attached, no extra lock trigger is created.
...
begin
  if old.status='completed' and new.status='scheduled' then
    raise exception using errcode='22023', message='completed session cannot return to scheduled';
  end if;
  ...
  if old.roster_locked_at is not null and new.roster_locked_at is distinct from old.roster_locked_at then
    raise exception using errcode='22023', message='roster lock cannot be changed';
  end if;
  ...
  if old.status='scheduled' and new.status='completed' and old.roster_locked_at is null then
    new.roster_locked_at := coalesce(new.roster_locked_at, now());
  end if;
...
begin
  ...
  if p_session_id is null then
    ...
    values (p_owner_id,p_class_id,v_class_name,p_start_at,p_end_at,'scheduled',nullif(btrim(p_memo),''),null)
  ...
  if v_old.status='completed' and p_status='scheduled' then
    raise exception using errcode='22023', message='completed session cannot return to scheduled';
  end if;
  ...
  ) then raise exception using errcode='22023', message='illegal session transition'; end if;
  ...
  v_completed_at := case when p_status='completed' then coalesce(v_old.completed_at, now()) else null end;
  update public.spokedu_master_sessions set
    ...
    roster_locked_at=case
      when p_status='completed' then coalesce(v_old.roster_locked_at, now())
      else v_old.roster_locked_at
    end
  where id=p_session_id returning id into v_id;
  return v_id;
end $$;

revoke all on function public.spokedu_master_save_session(uuid,uuid,uuid,timestamptz,timestamptz,text,text,jsonb,jsonb)
  from public, anon, authenticated;
grant execute on function public.spokedu_master_save_session(uuid,uuid,uuid,timestamptz,timestamptz,text,text,jsonb,jsonb)
  to service_role;

create or replace function public.spokedu_master_guard_session_roster_lock()
returns trigger language plpgsql set search_path = pg_catalog, public as $$
begin
  if old.status='completed' and new.status='scheduled' then
    raise exception using errcode='22023', message='completed session cannot return to scheduled';
...
drop trigger if exists spokedu_master_sessions_roster_lock on public.spokedu_master_sessions;

do $$
declare v_has_lock_trigger boolean;
begin
  select exists (
    select 1
    from pg_trigger t
    ...
    where t.tgrelid = 'public.spokedu_master_sessions'::regclass
      ...
      and pg_get_functiondef(p.oid) like '%roster_locked_at%'
      and (
        pg_get_functiondef(p.oid) like '%new.roster_locked_at := now()%'
        or pg_get_functiondef(p.oid) like '%new.roster_locked_at := coalesce(new.roster_locked_at, now())%'
      )
  ) into v_has_lock_trigger;

  if not v_has_lock_trigger then
    execute $sql$
      create trigger spokedu_master_sessions_roster_lock
      before update on public.spokedu_master_sessions
      for each row execute function public.spokedu_master_guard_session_roster_lock()
    $sql$;
  end if;
end $$;
