...
- 홈 화면의 깨진 한글 문구를 다시 제거하고, 첫 화면 메시지를 `수업 준비는 쉽게, 수업은 더 몰입감 있게` 기준으로 정리했다.
- 홈 카드 높이와 이미지 비율을 안정화했다. 히어로, 빠른 실행, 추천 패키지, 오늘 수업 카드가 각자 다른 크기로 튀지 않도록 최소 높이와 CTA 위치를 맞췄다.
...
- PWA 설치 카드의 깨진 문구를 수정하고, 설치 상태/오프라인 캐시 안내가 구독자에게 자연스럽게 보이도록 정리했다.
