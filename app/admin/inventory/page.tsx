...
export default function AdminInventoryPage() {
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden font-sans text-slate-900 relative">
      ...
      {/* 사이드바 수정: 헤더 부분 겹침 방지 */}
      <aside className={`
        fixed md:relative top-0 left-0 h-full bg-white border-r border-slate-200 flex flex-col shrink-0 z-[120] shadow-xl md:shadow-sm transition-transform duration-300
      ...
      `}>
        <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-white shrink-0 h-16">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            {/* hidden sm:block을 통해 작은 화면에서는 글자 숨김 */}
            <h2 className="text-xs font-black text-slate-900 whitespace-nowrap overflow-hidden hidden sm:block">교구 리스트</h2>
          </div>
          <button onClick={() => openCatalogModal()} className="p-1.5 bg-slate-50 hover:bg-indigo-50 rounded-lg text-indigo-600 cursor-pointer transition-colors shrink-0">
            <Plus size={16} />
          </button>
        </div>
      ...
      <main className={`flex-1 flex flex-col h-full relative transition-colors ${isDraggingOver ? 'bg-indigo-50' : 'bg-[#F8FAFC]'}`} onDragOver={(e) => { e.preventDefault(); if (selectedTeacher) setIsDraggingOver(true); }} onDragLeave={() => setIsDraggingOver(false)} onDrop={handleDrop}>
        ...
        {/* 메인 헤더 수정: SPOKEDU 텍스트와 드롭다운 겹침 방지 */}
        <header className="h-16 px-4 md:px-6 flex items-center justify-between sticky top-0 z-[100] bg-white/80 backdrop-blur-md border-b border-slate-200 shrink-0">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <button onClick={() => setIsMobileSidebarOpen(true)} className="md:hidden p-2 hover:bg-slate-100 rounded-lg text-slate-600 cursor-pointer shrink-0">
              <Menu size={20} />
            </button>
            
            {/* SPOKEDU 텍스트 영역 */}
            <div className="flex flex-col text-left shrink-0">
              <p className="text-[10px] font-bold text-indigo-600 leading-none tracking-tighter uppercase">SPOKEDU</p>
              <h1 className="text-[11px] font-black text-slate-900 leading-tight hidden xs:block">INVENTORY</h1>
            </div>

            <div className="relative z-[150] ml-2 md:ml-4 flex-1 max-w-[280px]">
              <button onClick={() => setIsTeacherDropdownOpen(!isTeacherDropdownOpen)} className="flex items-center gap-2 md:gap-3 px-3 py-2 bg-white border border-slate-200 rounded-xl hover:shadow-md cursor-pointer w-full overflow-hidden">
                <UserCircle2 size={16} className={`shrink-0 ${selectedTeacher ? 'text-indigo-600' : 'text-slate-400'}`} />
                <div className="text-left flex-1 min-w-0">
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none hidden sm:block">선생님 리스트</p>
                  <p className="text-xs font-black text-slate-800 truncate">{selectedTeacher ? selectedTeacher.name : '선생님 선택'}</p>
                </div>
          ...
          {selectedTeacher && <div className="hidden lg:block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Active Kit: <span className="text-indigo-600 text-sm ml-1">{teacherInventory.length}</span></div>}
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 min-h-0">
          {!selectedTeacher ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-4">
              ...
              <h2 className="text-lg font-bold">강사를 먼저 선택하세요</h2>
            </div>
          ) : (
              ...
              {teacherInventory.map((item) => (
                <div key={item.id} onClick={() => setDetailItem(item)} className="bg-white rounded-[24px] md:rounded-[32px] shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-slate-200 cursor-pointer">
                  ...
                  <div className="p-4 md:p-6" onClick={e => e.stopPropagation()}>
                    <h3 className="text-lg md:text-xl font-black text-slate-800 mb-3 md:mb-4 truncate leading-tight">{item.name}</h3>
                    <div className="flex items-center gap-2">
                        <div className="flex-1 bg-slate-50 rounded-2xl p-1 md:p-1.5 border border-slate-100 shadow-inner">
                            <input 
                                type="number" className="w-full bg-transparent text-center font-black text-xl md:text-2xl text-slate-800 outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" 
      ...
      {detailItem && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-2 md:p-4 bg-black/90 backdrop-blur-sm animate-in fade-in" onClick={() => setDetailItem(null)}>
          <div className="w-full max-w-md bg-[#121212] rounded-[24px] md:rounded-[32px] overflow-hidden shadow-2xl animate-in zoom-in-95 text-white border border-slate-800 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-6 md:p-8 pb-4 md:pb-6 border-b border-slate-800 flex gap-4 md:gap-6 items-center shrink-0">
            ...
            <div className="p-6 md:p-8 space-y-6 md:space-y-8 overflow-y-auto custom-scrollbar flex-1">
                <div className="bg-[#1a1a1a] p-4 md:p-5 rounded-2xl border border-slate-800">
                    <h4 className="text-[10px] md:text-[11px] font-black text-emerald-400 uppercase mb-2 md:mb-3 flex items-center gap-2 text-left"><CheckSquare size={14} /> 핵심 포인트</h4>
                    <p className="text-sm md:text-[15px] font-bold text-slate-200 whitespace-pre-line leading-relaxed text-left">{detailItem.key_points}</p>
                </div>
                <div>
                    <h4 className="text-[10px] md:text-[11px] font-black text-blue-400 uppercase mb-3 md:mb-4 flex items-center gap-2 text-left"><ListOrdered size={14} /> 활용 방법</h4>
                    <div className="space-y-3 md:space-y-4">
                        {detailItem.usage_examples?.split('\n').map((line: string, i: number) => (
                            <div key={i} className="flex gap-3 md:gap-4 items-start text-left">
                                ...
                                <p className="text-sm md:text-[15px] text-slate-300 font-bold leading-snug break-all text-left">{renderTextWithLinks(line)}</p>
                            </div>
                        ))}
                    </div>
                </div>
