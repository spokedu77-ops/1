...
export default function NoticePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F9FBFF] pb-32 text-slate-900 w-full font-sans overflow-x-hidden text-left">
      <style>{`
      ...
      <header className="sticky top-0 z-40 bg-white/70 backdrop-blur-xl border-b border-slate-200/50 px-6 h-16 flex items-center justify-center">
        <div className="max-w-2xl w-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="bg-indigo-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter">Admin</div>
          </div>
          <button onClick={() => fetchNotices()} className={`p-2 rounded-full hover:bg-slate-100 transition-all ${loading ? 'animate-spin' : ''}`}>
        ...
        <div className="flex gap-2 mb-8 overflow-x-auto no-scrollbar">
          {CATEGORIES.map((cat) => (
            <button key={cat.id} onClick={() => setActiveTab(cat.id)}
              className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all border shrink-0
                  ...
                  <div className={`transition-all duration-300 ease-in-out ${isExpanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="px-6 pb-6 space-y-4">
                      ...
                      <div className="flex justify-end gap-2">
                        <button onClick={(e) => openEdit(notice, e)} className="flex items-center gap-1.5 text-slate-500 hover:text-indigo-600 text-xs font-bold px-4 py-2 hover:bg-indigo-50 rounded-full transition-all">
                          <Edit3 size={14} /> 수정
                        </button>
                        <button onClick={(e) => handleDelete(notice.id, e)} className="flex items-center gap-1.5 text-slate-500 hover:text-rose-600 text-xs font-bold px-4 py-2 hover:bg-rose-50 rounded-full transition-all">
                          <Trash2 size={14} /> 삭제
                        </button>
                      </div>
                    </div>
      ...
      <button onClick={() => { setEditingId(null); setForm({ title: '', content: '', category: 'general', is_pinned: false }); setIsModalOpen(true); }}
        className="fixed bottom-10 right-10 w-16 h-16 bg-slate-900 text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 active:scale-90 transition-all z-50">
        <Plus size={32} />
      </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[32px] p-8 shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start mb-8 text-left">
              <div>
                <p className="text-indigo-600 text-[10px] font-black uppercase tracking-widest mb-1">{editingId ? 'Edit' : 'New'} Notice</p>
                <h2 className="text-2xl font-black text-slate-900">공지 {editingId ? '수정' : '작성'}</h2>
              </div>
            ...
            <div className="space-y-6 text-left">
              ...
              <div className="space-y-4">
                <input className="w-full bg-slate-50 border-none p-4 rounded-2xl outline-none font-bold text-slate-800 placeholder-slate-300 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all"
                  placeholder="제목" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
                <textarea className="w-full bg-slate-50 border-none p-4 rounded-2xl outline-none h-40 resize-none font-medium text-slate-700 placeholder-slate-300 focus:bg-white focus:ring-2 focus:ring-indigo-500/20 transition-all leading-relaxed"
              ...
              <button onClick={handleSave} className="w-full bg-indigo-600 text-white py-5 rounded-[24px] font-black text-lg hover:bg-indigo-700 shadow-xl shadow-indigo-200 transition-all">
