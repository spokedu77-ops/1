...
type Settings = {
  ...
  /** basic 1번 · 공간 방향: 기본(흰 화살표) | 색상(위 빨·우 노·좌 초·아래 파 고정) */
  ...
  /** stroop 1번: 음성(기본) | SPOMAT 이동(내부 후보) */
  ...
  /** 스테이지당 시간(초) */
  flowDuration: number;
  /** DIVE 스테이지 구성: sequential(순차 도입) | random(1스테이지 혼합) */
  ...
  /** reactTrain level 6 (두더지 잡기): 본 활동 뒤 15초 가속 보너스타임 */
...
const defaultSettings: Settings = {
  ...
  stroopArrowMode: 'basic' as const,
  stroopWordDifficulty: 'basic' as const,
  stroopArrowResponse: 'voice' as const,
...
export function settingsToExitResume(s: Settings): TrainingExitResume {
  return {
    ...
    launch: {
      ...
      intervalWork: s.intervalWork,
      intervalRest: s.intervalRest,
      intervalSets: s.intervalSets,
      ...
      flankerExtremeMode: s.flankerExtremeMode,
      flankerArrowMode: s.flankerArrowMode,
      stroopWordMode: s.stroopWordMode,
      stroopArrowMode: s.stroopArrowMode,
      stroopWordDifficulty: s.stroopWordDifficulty,
      stroopArrowResponse: s.stroopArrowResponse,
...
export default function MemoryGameApp({
  ...
  useEffect(() => {
    ...
    if (typeof fromAuto === 'string' && fromAuto.trim()) {
      ...
      setSettings((s) => ({
        ...s,
        variantColorTheme: fromAuto,
        basicNumberOverlay: fromAuto === 'color' ? s.basicNumberOverlay : 'none',
      }));
      return;
    }
    setSettings((s) => ({ ...s, variantColorTheme: 'color', basicNumberOverlay: 'none' }));
  ...
  const flankerThemeLevel =
    settings.mode === 'flanker' &&
    (settings.level === 2 || settings.level === 6 || (settings.level === 3 && settings.flankerExtremeMode !== 'arrow'));
  ...
  const readiness = useMemo(
    () => evaluateAssetReadiness({
      ...
      theme:
        settings.mode === 'flanker' && settings.level === 3 && settings.flankerExtremeMode === 'arrow'
          ? 'color'
          : settings.variantColorTheme,
      simonPoleCount: settings.simonPoleCount,
      loadStatus: activeAssetStatus,
      slides: variantSignalSlides ?? [],
    }),
    [settings.mode, settings.level, settings.flankerExtremeMode, settings.variantColorTheme, settings.simonPoleCount, activeAssetStatus, variantSignalSlides]
  );
  ...
  const set = useCallback((key: keyof Settings, value: unknown) => {
    setSettings((s) => {
      ...
      if (key === 'level' && s.mode === 'reactTrain' && typeof value === 'number') {
        const mapped = resolveReactTrainUiLevel(value);
        if (mapped.moleLookMode) next.moleLookMode = mapped.moleLookMode;
  ...
  const deliverSessionResult = useCallback((
    ...
    setResult({ cfg, elapsedMs, colorCounts, endedEarly: meta?.endedEarly === true });
    setScreen('result');
    onComplete?.(payload);
  }, [embed, onComplete]);
  ...
  const { getProgress } = useTrainingTimer({
    ...
    flankerExtremeMode: settings.flankerExtremeMode,
    flankerArrowMode: settings.flankerArrowMode,
    stroopWordMode: settings.stroopWordMode,
    stroopArrowMode: settings.stroopArrowMode,
    stroopWordDifficulty: settings.stroopWordDifficulty,
    stroopArrowResponse: settings.stroopArrowResponse,
    stroopWordResponse: settings.stroopWordResponse,
    stroopWordRuleMode: settings.stroopWordRuleMode,
  ...
  const startSession = useCallback(
    (cfg: Settings = settings) => {
      ...
      // O4: preserve autoLaunch/user intervalSets (do not force 4)
      setSettings(cfg);
      ...
      sessionSlidesRef.current =
        readiness.requirement.minimumCount > 0 && readiness.status === 'ready'
          ? readiness.usableAssets.map((s) => ({ ...s }))
          : undefined;
    ...
    [disableBgm, readiness, settings, spomoveBgmList, spomoveBgmLoading]
  );
  ...
  const stop = useCallback(() => {
    ...
    if (shouldShowPartialResult) {
      ...
      const elapsedMs = sessionStartMsRef.current > 0 ? performance.now() - sessionStartMsRef.current : 0;
      const counts =
        totalColorStimulusCount(stoppedColorCounts) > 0 ? stoppedColorCounts : null;
      deliverSessionResult(cfg, elapsedMs, counts, { endedEarly: true });
      return;
    }
    if (autoLaunch) {
  ...
  const startSportsArenaBgm = useCallback(() => {
    if (disableBgm) return;
    const path = flowBgmPathRef.current;
    if (!path) return;
    try {
      const url = getPublicUrl(path);
      if (!bgmPlayerRef.current) bgmPlayerRef.current = new BgmPlayer();
      ...
      // BGM failures must not block the Unity session.
    }
  }, [disableBgm]);

  const stopSportsArenaBgm = useCallback(() => {
    bgmPlayerRef.current?.fadeOut(220);
  }, []);
  ...
  if (autoLaunch && !sessionLaunchedRef.current && (screen === 'home' || screen === 'setup' || screen === 'result')) {
    return (
      <div
        style={{
          position: 'fixed',
        ...
        <div style={S.scroll}>
          <div style={{ ...S.card, maxWidth: '34rem' }}>
            ...
            <div style={S.sec}>
              ...
              {settings.mode === 'stroop' && (settings.level === 1 || settings.level === 2) ? (
                <div style={{ marginTop: '1.15rem', paddingTop: '1.15rem', borderTop: '1px solid var(--border)' }}>
                  <div style={{ fontSize: '0.95rem', fontWeight: 800, color: 'var(--text)', marginBottom: '0.55rem' }}>난이도</div>
                  <div style={{ display: 'flex', gap: '0.4rem' }}>
                    {([
                      { id: 'basic' as const, label: '보통 · 기본' },
                      { id: 'bg' as const, label: '어려움 · 배경 간섭' },
                    ]).map((opt) => {
                      const active =
                        settings.level === 1
                          ? settings.stroopArrowMode === opt.id
                          : settings.stroopWordDifficulty === opt.id;
                      return (
                        <button
                          key={opt.id}
                          type="button"
                          onClick={() => {
                            if (settings.level === 1) set('stroopArrowMode', opt.id);
                            else set('stroopWordDifficulty', opt.id);
                          }}
                          style={{
                            flex: 1,
            ...
            {settings.mode === 'flow' && (
              <>
                    ...
                    {DIVE_THEME_UI.map(({ id, label }) => {
                      ...
                      return (
                        <button
                          key={id}
                          type="button"
                          onClick={() => setSettings((s) => ({ ...s, diveEnvironmentTheme: id, flowDuration: isDiveActionMoveUnityTheme(id) && ![15, 20, 25, 30, 35].includes(s.flowDuration) ? 20 : s.flowDuration }))}
                          style={{
                            flex: '1 1 88px',
                ...
                <div style={S.sec}>
                  ...
                  {isDiveActionMoveUnityTheme(settings.diveEnvironmentTheme) ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                      {([
                        ['side', '↔️', 'SIDE', '좌·우 이동'],
                        ['jump', '⬆️', 'JUMP', '장애물 뛰어넘기'],
                        ['duck', '⬇️', 'DUCK', '숙여 통과하기'],
                  ...
                  <div style={{ display: isDiveActionMoveUnityTheme(settings.diveEnvironmentTheme) ? 'none' : 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                ...
                <div style={{ ...S.sec, display: isDiveActionMoveUnityTheme(settings.diveEnvironmentTheme) ? 'none' : undefined }}>
                  {stepNum(5, '모션 게이트')}
                  <p style={{ fontSize: '0.86rem', color: 'var(--text-muted)', marginBottom: '0.75rem', lineHeight: 1.55 }}>
                ...
                {!isDiveActionMoveUnityTheme(settings.diveEnvironmentTheme) && settings.flowFeatures.has('colorGate') && (
                  <div style={S.sec}>
                    {stepNum(5, '모션 게이트 옵션')}
                ...
                <div style={S.sec}>
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
                    {(!isDiveActionMoveUnityTheme(settings.diveEnvironmentTheme) && settings.flowFeatures.has('colorGate') && settings.flowFeatures.size === 1
                      ? [45, 60, 90, 120, 150]
                      : [15, 20, 25, 30, 35]
                    ).map((n) => (
                      <button
                ...
                <div style={S.sec}>
                  ...
                  {isDiveActionMoveUnityTheme(settings.diveEnvironmentTheme) ? <p style={{ margin: '0.65rem 0 0', fontSize: '0.82rem', fontWeight: 800, color: 'var(--text-muted)', wordBreak: 'keep-all' }}>예상 활동 시간 <span style={{ whiteSpace: 'nowrap' }}>{formatDiveActionMoveDuration(diveActionMoveDurationSec({ side: settings.sportsArenaFeatures.has('side'), jump: settings.sportsArenaFeatures.has('jump'), duck: settings.sportsArenaFeatures.has('duck'), bonus: settings.flowIncludeBonus }, settings.flowDuration))}</span>{formatDiveActionMoveDurationDetail({ side: settings.sportsArenaFeatures.has('side'), jump: settings.sportsArenaFeatures.has('jump'), duck: settings.sportsArenaFeatures.has('duck'), bonus: settings.flowIncludeBonus }, settings.flowDuration) ? <span style={{ display: 'block', marginTop: 4, fontSize: '0.72rem', fontWeight: 700, whiteSpace: 'nowrap' }}>{formatDiveActionMoveDurationDetail({ side: settings.sportsArenaFeatures.has('side'), jump: settings.sportsArenaFeatures.has('jump'), duck: settings.sportsArenaFeatures.has('duck'), bonus: settings.flowIncludeBonus }, settings.flowDuration)}</span> : null}</p> : null}
                </div>
                <div style={S.sec}>
                ...
                {(settings.mode === 'basic' && ((settings.level === 7 && settings.relativeCompassEnabled) || (settings.level === 8 && settings.shapeCompletionEnabled))) || (settings.mode === 'reactTrain' && (settings.level === 5 || reactTrainEngineLevelForUi(settings.level) === 9 || reactTrainEngineLevelForUi(settings.level) === 14)) ? null : spatialBasicRandomSpeed ? (
                  ...
                  <div style={S.sec}>
                    ...
                    <SpeedSelector
                      ...
                      showPresets={false}
                      max={settings.flowFeatures.has('colorGate') ? 10 : 6}
                    />
                  </div>
                )}
  ...
  if (screen === 'flow') {
    ...
    const stages = buildStages(selectedModules, settings.flowDuration, {
      layout: settings.flowLayout,
      includeBonus: settings.flowIncludeBonus,
    });
  ...
  if (screen === 'result' && result) {
    ...
    return (
      <TrainingResultScreen
        ...
        onBack={goToList}
        diveActionMove={
          cfg.mode === 'flow' && cfg.level === 1
            ? {
                completed: !result.endedEarly,
                stageDurationSec: cfg.flowDuration,
                environmentTheme: cfg.diveEnvironmentTheme,
                sportsArenaFeatures: [...cfg.sportsArenaFeatures],
                flowFeatures: [...cfg.flowFeatures],
                flowIncludeBonus: cfg.flowIncludeBonus,
