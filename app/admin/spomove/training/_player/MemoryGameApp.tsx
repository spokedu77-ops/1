...
type Settings = {
  ...
  /** ???????????????????????????????????????????? */
  flowDuration: number;
  /** ???????????????????????????????諛몃마嶺뚮?????????????硫λ젒????????????????????遺얘턁??????얜Ŧ堉??????⑤뜪?????????????????????????癲?????????????????????????????????????????????????????????????????????????????????????????????????????????????????ㅻ깹??????????????????????????????? URL */
  flowBgImageUrl: string;
  /** Flow ????????????????????????????????????? classic = ???????????????? plus = flow-lab enhanced */
...
export type MemoryGameAutoLaunch = {
  ...
  flowFeatures?: string[];
  /** Flow 2.0: ?????????????????????????諛몃마嶺뚮?????????????硫λ젒????????????????????遺얘턁??????얜Ŧ堉??????⑤뜪?????????????????????????癲?????????????????????????????????????????????????????????????????????????????????????????????????????????????????ㅻ깹?????????????????????????????????????????????????????ㅻ깹???????????????????????????????????????????????*/
  flowColorTheme?: 'default' | 'space' | 'neon' | 'ocean';
  /** Flow 2.0: ?????????????????????????????????????? */
  flowDuration?: number;
  /** Flow 2.0: ?????????????????????????諛몃마嶺뚮?????????????硫λ젒????????????????????遺얘턁??????얜Ŧ堉??????⑤뜪?????????????????????????癲?????????????????????????????????????????????????????????????????????????????????????????????????????????????????ㅻ깹??????????????????????????????? URL */
  flowBgImageUrl?: string;
  /** Flow ?????????????????????????????????????*/
  flowVisualVariant?: FlowVisualVariant;
  /** ????????????????????????1?? ??????????????????????????????????????????????*/
...
export default function MemoryGameApp({
  ...
  const staticDivePanoramaUrl = '/spomove/dive/environments/space/panorama.webp';
  ...
  const divePanoramaLowUrl = diveSpaceEntry
    ? (getDivePreviewUrl(diveSpaceEntry.panoramaLowPath) ?? staticDivePanoramaUrl)
    : staticDivePanoramaUrl;
  ...
  useEffect(() => {
    ...
    // ??????????????????????????꾩룆梨띰쭕?뚢뵾??????????????嶺뚮죭?댁젘??????????????????????釉먮폁???????????????????살몝???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????+ ???????????????????????????諛몃마嶺뚮?????????????硫λ젒????????????????????遺얘턁??????얜Ŧ堉??????⑤뜪?????????????????????????癲??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????怨뺤떪????????????????????????????嫄????????????????????? (flow ????????????????????????????????????????????ㅻ깹??????????????????????
  ...
  useEffect(() => {
    ...
    // ??????????????????????????꾩룆梨띰쭕?뚢뵾??????????????嶺뚮죭?댁젘??????????????????????釉먮폁???????????????????살몝???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????+ ???????????????????????????諛몃마嶺뚮?????????????硫λ젒????????????????????遺얘턁??????얜Ŧ堉??????⑤뜪?????????????????????????癲??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????怨뺤떪????????????????????????????嫄?????????????????????
    const prevOverflow = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
    const id = requestAnimationFrame(() => {
      document.documentElement.requestFullscreen?.().catch(() => {});
          ...
          <div style={{ ...S.card, maxWidth: '34rem' }}>
            ...
            {settings.mode === 'flow' && (
              <>
                {/* DIVE ????????????????????????????????????????????????????*/}
                <div style={S.sec}>
                  {stepNum(3, "DIVE visual mode")}
                  <div style={{ display: 'flex', gap: '0.45rem' }}>
                    {([
                      { v: 'classic' as FlowVisualVariant, label: 'DIVE', desc: 'Basic 3D training' },
                      { v: 'plus' as FlowVisualVariant, label: 'DIVE+', desc: 'Panorama and enhanced visuals' },
                    ]).map(({ v, label, desc }) => {
                ...
                <div style={S.sec}>
                  {stepNum(4, "Background theme")}
                  <div style={{ display: 'flex', gap: '0.45rem', flexWrap: 'wrap' }}>
                    {(
                      [
                        { key: 'default', label: 'Default', desc: 'Basic DIVE theme' },
                        { key: 'space',   label: 'Space',   desc: 'Dark space theme' },
                        { key: 'neon',    label: 'Neon',    desc: 'Neon theme' },
                        { key: 'ocean',   label: 'Ocean',   desc: 'Ocean theme' },
                ...
                {settings.flowVisualVariant === 'plus' && (
                  <div style={{ ...S.sec, padding: '0.6rem 0.75rem', borderRadius: '0.75rem', background: 'rgba(34,211,238,0.07)', border: '1px solid rgba(34,211,238,0.25)', fontSize: '0.8rem', color: '#22d3ee' }}>
                    DIVE+????????????????????????????꾩룆梨띰쭕?뚢뵾??????????????嶺뚮죭?댁젘??????????????????????釉먮폁???????????????????살몝????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
                  </div>
                )}
                {settings.flowVisualVariant !== 'plus' && (
                <div style={S.sec}>
                  <p style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text)', marginBottom: '0.4rem' }}>
                    ?????????????????????????諛몃마嶺뚮?????????????硫λ젒????????????????????遺얘턁??????얜Ŧ堉??????⑤뜪?????????????????????????癲?????????????????????????????????????????????????????????????????????????????????????????????????????????????????ㅻ깹??????????????????????????????? URL <span style={{ fontWeight: 400, color: 'var(--text-muted)' }}>(???????????????</span>
                  </p>
                  ...
                  <input
                    type="text"
                    placeholder="https://... ?????/images/bg.jpg"
                ...
                <div style={S.sec}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                    {(
                      [
                        { key: 'punch', icon: 'P', label: 'Punch', desc: 'Punch obstacles.' },
                        { key: 'duck', icon: 'D', label: 'Duck', desc: 'Duck under obstacles.' },
                        { key: 'reach', icon: 'R', label: 'Reach', desc: 'Break the wall.' },
  ...
  if (screen === 'flow') {
    ...
    // DIVE+: enhanced ???????????????????????????+?????????????/ classic+space: legacy ???????????????????????????+?????????????
