      ...
      if (cloneMode === 'fixed') {
        ...
        for (let i = 1; i <= cloneRounds; i++) {
          ...
          sessionsToCopy.push({
            ...
            start_at: newStart.toISOString(),
            end_at: newEnd.toISOString(),
            // 다음 회차(opened) 생성 시 이전 회차 피드백 텍스트가 그대로 이어붙지 않도록 초기화
            students_text: null,
            feedback_fields: {},
            photo_url: [],
            file_url: [],
        ...
        <nav className="border-b px-2 sm:px-4 py-2 sm:py-3 bg-white flex justify-between items-center z-50">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <Link
              ...
              className="h-8 sm:h-9 px-3 sm:px-4 bg-white border border-slate-200 text-slate-700 text-[10px] sm:text-xs font-black rounded-lg flex items-center shadow-md hover:bg-slate-50"
            >
              LIST
            </Link>
