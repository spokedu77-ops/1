...
export default function AdminCurriculumPage() {
  ...
  /** 가이드라인 1~12번 칩에 표시할 교구명 (DB 미입력 시 N번 교구) */
  ...
  const selectedEquipmentDisplayName = equipmentGuideChipLabelByNumber.get(selectedEquipmentNumber) ?? `${selectedEquipmentNumber}번 교구`;
                     ...
                     <div className="w-full grid grid-cols-6 gap-2 auto-rows-fr">
                       {EQUIPMENT_GUIDE_NUMBERS.map((num) => {
                         ...
                         <button key={num} type="button" onClick={() => setSelectedEquipmentNumber(num)}
                           ...
                           className={`min-h-[52px] sm:min-h-[56px] rounded-lg sm:rounded-xl flex items-center justify-center transition-all border font-bold text-[10px] sm:text-xs leading-tight px-1.5 py-2 touch-manipulation
          ...
          <form onSubmit={handleEquipmentSubmit} className="relative bg-white w-full max-w-lg rounded-[32px] p-8 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto text-left">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-black text-slate-400 uppercase">교구</label>
                  <select className="w-full bg-slate-100 p-4 rounded-2xl outline-none" value={equipmentForm.number} onChange={e => setEquipmentForm(f => ({ ...f, number: Number(e.target.value) }))}>
                    {EQUIPMENT_GUIDE_NUMBERS.map((n) => (
                      <option key={n} value={n}>
                        {equipmentGuideChipLabelByNumber.get(n) ?? `${n}번 교구`}
                      </option>
                    ))}
                  </select>
                </div>
