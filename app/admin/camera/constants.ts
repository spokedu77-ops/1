/**
 * Shared constants for the SPOKEDU camera activity.
 */
...
export const ACTIVE_CAMERA_MODE_IDS = ['speed', 'sequence', 'shape'] as const satisfies readonly CameraModeId[];

export const MAX_CAMERA_PARTICIPANTS = 3;
export const RECOMMENDED_CAMERA_PARTICIPANTS = 2;
...
export const MODE_META: Record<CameraModeId, { label: string; emoji: string }> = {
  speed: { label: '반응 깨우기', emoji: '⚡' },
  sequence: { label: '순서 따라가기', emoji: '🔢' },
...
/** MediaPipe hand/arm landmarks used as touch points. */
export const TOUCH_IDX = [15, 16, 19, 20, 17, 18];

export const POSES = [
  { name: '양팔 벌리기', check: (lm: { x: number; y: number }[]) => Math.abs(lm[15].x - lm[16].x) > 0.55 },
  { name: '양손 머리 위', check: (lm: { x: number; y: number }[]) => lm[15].y < lm[11].y - 0.10 && lm[16].y < lm[12].y - 0.10 },
...
export const MIRROR_POSES = [
  { name: '양팔 벌리기', instruction: '양팔을 옆으로 크게 펴세요', check: (lm: { x: number; y: number }[]) => Math.abs(lm[15].x - lm[16].x) > 0.50 },
  { name: '양손 머리 위로', instruction: '두 손을 머리 위로 올리세요', check: (lm: { x: number; y: number }[]) => lm[15].y < lm[11].y - 0.08 && lm[16].y < lm[12].y - 0.08 },
