        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredSessions.map((s) => {
              ...
              const isFinished = s.status === 'finished' && pureContent.length > 0;
                ...
                <div key={s.id} className="bg-white rounded-[40px] p-8 shadow-sm border border-slate-200 flex flex-col hover:shadow-xl transition-all duration-300">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <span className={`text-[11px] font-black px-3 py-1.5 rounded-xl uppercase tracking-widest ${
                      isVerified ? 'bg-blue-600 text-white shadow-md shadow-blue-100' : 
                      isFinished ? 'bg-emerald-500 text-white shadow-md shadow-emerald-100' : 
                  ...
                  <div className="mt-auto flex gap-3">
                    <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/report/${s.id}`); alert('리포트 링크가 복사되었습니다.'); }} disabled={isEmpty} className={`p-4 rounded-3xl transition-all cursor-pointer ${!isEmpty ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-slate-50 text-slate-200 cursor-not-allowed'}`} title="링크 복사"><Send size={20} /></button>
            ...
            <div className="flex-1 overflow-y-auto p-10 space-y-10 bg-slate-50/50">
              ...
              {photoUrls.length > 0 && (
                <div className="grid grid-cols-3 gap-4">
                  {photoUrls.map((url, i) => (
                    <div key={i} className="relative aspect-square rounded-[24px] overflow-hidden border-2 border-white shadow-sm hover:scale-105 transition-transform cursor-zoom-in group" onClick={() => setZoomedImg(url)}>
                      <img src={url} className="w-full h-full object-cover" alt="수업 사진" />
                      <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Maximize2 size={20} className="text-white" /></div>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="space-y-4">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Final Review</label>
                <textarea className="w-full h-80 bg-white border-none shadow-inner rounded-[32px] p-8 text-base leading-relaxed text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100 transition-all" value={feedback} onChange={(e) => setFeedback(e.target.value)} />
              </div>
            </div>
