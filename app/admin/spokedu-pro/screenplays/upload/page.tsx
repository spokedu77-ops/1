...
export default function AdminScreenplayUploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/spokedu-pro/screenplays/import', { method: 'POST', body: formData });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6 lg:p-10 space-y-8 max-w-5xl mx-auto">
      <div className="flex items-center gap-4">
        <Link href="/admin/spokedu-pro" className="flex items-center gap-1.5 text-slate-400 hover:text-white text-sm transition-colors">
      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-5 space-y-3">
        <p className="text-sm font-bold text-slate-300 flex items-center gap-2">
          <FileSpreadsheet className="w-4 h-4 text-orange-400" /> CSV 포맷 (UTF-8)
        </p>
        <p className="text-xs text-slate-500 font-mono bg-slate-900 px-3 py-2 rounded-lg">{CSV_HEADERS.join(', ')}</p>
        <div className="text-xs text-slate-500 space-y-1">
          ...
          <p>업로드 후 <code className="bg-slate-800 px-1 rounded">is_published=false</code>로 저장. DB에서 true로 변경하세요.</p>
        </div>
      </div>
      ...
      >
        <input id="screenplay-file-input" type="file" accept=".csv" className="hidden"
      ...
      {previewRows.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">미리보기 (최대 15행)</p>
          <div className="overflow-x-auto rounded-xl border border-slate-700">
            <table className="text-xs w-full">
              <thead className="bg-slate-800 border-b border-slate-700">
                <tr>{previewHeaders.map((h) => <th key={h} className="px-3 py-2.5 text-left font-bold text-slate-400 whitespace-nowrap">{h}</th>)}</tr>
              </thead>
              <tbody>
                {previewRows.map((row, i) => (
                  <tr key={i} className="border-b border-slate-800 hover:bg-slate-800/40">
                    {previewHeaders.map((h) => <td key={h} className="px-3 py-2 text-slate-300 max-w-[200px] truncate">{row[h] ?? ''}</td>)}
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <button type="button" onClick={handleUpload} disabled={!file || loading}
        ...
        {loading ? (
          ...
          <><Upload className="w-5 h-5" />DB에 반영</>
        )}
      </button>
    </div>
  );
