...
export function AssignProgramToSessionButton({ program, className }: { program: Program; className?: string }) {
  ...
  return (
    <>
      ...
      {open ? (
        <BottomSheet open title="수업에 배정" onClose={() => setOpen(false)}>
          <div className="pb-4">
            ...
            <div className="mt-4 space-y-2">
              {sessions.map((session) => (
                <button key={session.id} type="button" disabled={savingId !== null || session.status === 'cancelled'} onClick={() => void assign(session.id)} className="flex min-h-12 w-full items-center justify-between rounded-xl border border-slate-200 bg-white px-3 text-left disabled:opacity-40">
                  <span><strong className="block text-sm text-slate-800">{format(new Date(session.startAt), 'HH:mm')} {session.className}</strong><small className="text-slate-500">{session.programs.length}개 프로그램 · {session.status === 'completed' ? '완료' : session.status === 'cancelled' ? '취소' : '예정'}</small></span>
