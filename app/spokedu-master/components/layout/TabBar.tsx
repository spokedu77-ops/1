...
export function TabBar() {
  ...
  return (
    <nav className="sticky bottom-0 z-50 shrink-0 border-t px-3 py-2 sm:px-6 sm:pb-4 lg:hidden" style={{ borderColor: 'var(--spm-br2)' }} aria-label="SPOKEDU MASTER 주요 메뉴">
  ...
  return (
    <aside className="hidden w-[208px] shrink-0 border-r px-4 py-5 lg:block" style={{ borderColor: 'var(--spm-br2)', background: 'rgba(7,7,12,0.72)' }} aria-label="SPOKEDU MASTER 데스크톱 메뉴">
      ...
      <div className="mt-6 rounded-[14px] p-3" style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.18), rgba(16,185,129,0.12))', border: '1px solid var(--spm-br2)' }}>
        <p className="text-[11px] font-black" style={{ color: '#a5b4fc' }}>MASTER 구독</p>
        <p className="mt-2 text-[12px] font-semibold leading-5" style={{ color: 'var(--spm-t2)' }}>프로그램 라이브러리, SPOMOVE 실행, 수업 기록, 보호자 리포트를 하나의 흐름으로 운영합니다.</p>
      </div>
    </aside>
  );
}
