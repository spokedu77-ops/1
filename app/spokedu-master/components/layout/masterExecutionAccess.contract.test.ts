...
const classMode = read('app/spokedu-master/class-mode/[id]/page.tsx');
...
describe('MASTER execution screen access boundary', () => {
  ...
  it('does not render class-mode children before access is allowed', () => {
    ...
    expect(classMode).toContain('<ClassModeView programId={id} />');
  });

  it('uses the common shell guard for both SPOMOVE session and class-mode', () => {
    expect(shell).toContain('pathname.startsWith(`${basePath}/spomove/session`)');
    expect(shell).toContain('pathname.startsWith(`${basePath}/class-mode`)');
