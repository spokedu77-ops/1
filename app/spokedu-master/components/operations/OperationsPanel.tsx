...
function getRetryTypeLabel(type: string) {
  ...
  if (type === 'class-record-sync') return '수업 기록 저장 확인';
  return type;
}
          ...
          {operational.retryQueue.slice(0, compact ? 2 : 6).map((item) => (
            <div key={item.id} className="flex items-center gap-3 rounded-[12px] p-3" style={{ background: 'var(--spm-amb-a10)', border: '1px solid var(--spm-amb-a18)' }}>
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-1 block text-[10px] font-semibold" style={{ color: 'var(--spm-t3)' }}>{getRetryTypeLabel(item.type)} / 확인 후 목록에서 지울 수 있습니다</span>
              </span>
              <button type="button" onClick={() => removeRetry(item.id)} className="grid h-8 w-8 place-items-center rounded-[9px]" style={{ background: 'rgba(239,68,68,0.12)' }} aria-label="목록에서 지우기" title="목록에서 지우기">
