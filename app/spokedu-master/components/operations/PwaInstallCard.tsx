...
export function PwaInstallCard({ compact = false }: { compact?: boolean }) {
  ...
  const installLabel = promptEvent ? '설치하기' : '브라우저에서 추가';
  const installHelp = standalone
    ? '이미 독립 실행 환경에서 열려 있습니다.'
    : promptEvent
      ? '홈 화면 또는 앱 목록에서 SPOKEDU MASTER를 빠르게 다시 열 수 있습니다.'
      : '설치 버튼이 보이지 않으면 브라우저 메뉴에서 홈 화면에 추가를 선택하세요.';
        ...
        <div className="min-w-0 flex-1">
          ...
          {!compact ? (
            <p className="mt-2 text-[12px] font-medium leading-5" style={{ color: 'var(--spm-t3)' }}>
              홈 화면이나 앱 목록에서 빠르게 다시 열 수 있습니다. 수업 자료와 기록 기능은 인터넷 연결 상태에서 사용해 주세요.
            </p>
          ) : null}
        </div>
      </div>
