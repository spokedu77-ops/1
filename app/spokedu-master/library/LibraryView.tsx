    ...
    <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
      <Link href={`/spokedu-master/library/${featured.id}`} className="grid overflow-hidden rounded-[18px] p-5 active:scale-[0.99] md:grid-cols-[1fr_auto] md:items-end md:p-7" style={{ background: `linear-gradient(135deg, ${featured.colors[0]}, ${featured.colors[1]}, ${featured.colors[2]})`, boxShadow: '0 18px 42px rgba(99,102,241,0.2)' }}>
        <div>
          ...
          <p className="mt-2 max-w-[520px] text-[12px] font-semibold leading-5 text-white/65 md:text-[14px]">최근 수업 기록과 방향 전환 약점을 기준으로 추천합니다. 내 반에서 사용한 프로그램 {usedProgramIds.size}개가 쌓일수록 추천 정확도가 올라갑니다.</p>
...
function SubscriptionValueStrip() {
  const items = [['153', '프로그램'], ['PWA', '웹 실행'], ['누적', '성장 기록']];
...
function ProgramRail({ title, caption, programs, isPro, usedProgramIds, favorites, onFavorite }: { title: string; caption: string; programs: typeof PROGRAMS; isPro: boolean; usedProgramIds: Set<string>; favorites: string[]; onFavorite: (id: string) => void }) {
  return (
...
export default function LibraryView() {
  ...
  const filtered = useMemo(() => {
    ...
    return PROGRAMS.filter((program) => {
      ...
      const matchGrade = grade === '전체 학년' || program.grade.includes(grade);
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      ...
      <ProgramRail title="TOP 5" caption="이번 주 추천" programs={topPrograms} isPro={isPro} usedProgramIds={usedProgramIds} favorites={favorites} onFavorite={toggleFavorite} />
      <ProgramRail title="SPOMOVE 연결 수업" caption="웹 실행 포함" programs={spomovePrograms} isPro={isPro} usedProgramIds={usedProgramIds} favorites={favorites} onFavorite={toggleFavorite} />
      <ProgramRail title="18분 이내 빠른 수업" caption="대체 수업용" programs={quickPrograms} isPro={isPro} usedProgramIds={usedProgramIds} favorites={favorites} onFavorite={toggleFavorite} />
      ...
      <section className="px-[22px] sm:px-8 lg:px-10"><div className="mb-[14px] flex items-baseline justify-between"><h2 className="text-[18px] font-bold" style={{ fontFamily: 'var(--spm-font-display)' }}>전체 프로그램</h2><span className="text-[12px] font-medium" style={{ color: 'var(--spm-t3)' }}>{filtered.length}개</span></div>{filtered.length > 0 ? <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{filtered.map((program) => <ProgramListItem key={program.id} program={program} locked={program.isPro && !isPro} used={usedProgramIds.has(program.id)} favorite={favorites.includes(program.id)} onFavorite={() => toggleFavorite(program.id)} />)}</div> : <div className="rounded-[14px] p-5 text-center" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}><p className="text-[14px] font-bold" style={{ color: 'var(--spm-t)' }}>조건에 맞는 수업안이 없습니다.</p><p className="mt-1 text-[12px]" style={{ color: 'var(--spm-t3)' }}>필터와 검색어를 조금 바꿔보세요.</p></div>}</section>
