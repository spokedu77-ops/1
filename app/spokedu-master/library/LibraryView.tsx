...
function PosterCard({ program, rank, locked }: { program: (typeof PROGRAMS)[number]; rank: number; locked: boolean }) {
  return (
    <Link href={locked ? '/spokedu-master/profile' : `/spokedu-master/library/${program.id}`} className="relative h-[196px] w-[140px] shrink-0 overflow-hidden rounded-[14px] active:scale-[0.98]" style={{ background: `linear-gradient(135deg, ${program.colors[0]}, ${program.colors[1]}, ${program.colors[2]})` }}>
      <span className="absolute bottom-[-13px] left-[-4px] text-[54px] font-bold leading-none" style={{ fontFamily: 'var(--spm-font-display)', color: 'rgba(13,13,20,0.74)', WebkitTextStroke: '1px rgba(255,255,255,0.18)' }}>
        {rank}
      </span>
  ...
  return (
    <Link href={locked ? '/spokedu-master/profile' : `/spokedu-master/library/${program.id}`} className="relative flex h-full gap-3 rounded-[14px] p-3 active:scale-[0.99]" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br)' }}>
      ...
      <div className="min-w-0 flex-1 py-0.5">
        ...
        <h2 className="truncate text-[14px] font-bold" style={{ color: 'var(--spm-t)' }}>
          {program.title}
        </h2>
    ...
    <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
      <Link href={`/spokedu-master/library/${featured.id}`} className="grid overflow-hidden rounded-[18px] p-5 active:scale-[0.99] md:grid-cols-[1fr_auto] md:items-end md:p-7" style={{ background: `linear-gradient(135deg, ${featured.colors[0]}, ${featured.colors[1]}, ${featured.colors[2]})`, boxShadow: '0 18px 42px rgba(99,102,241,0.2)' }}>
        <div>
          ...
          <h2 className="mt-3 max-w-[580px] text-[28px] font-black leading-tight text-white md:text-[40px]" style={{ fontFamily: 'var(--spm-font-display)', letterSpacing: 0, wordBreak: 'keep-all' }}>
            {featured.title}
          </h2>
          <p className="mt-2 max-w-[520px] text-[12px] font-semibold leading-5 text-white/65 md:text-[14px]">
            3학년 A반의 최근 SPOMOVE 기록과 방향 전환 약점을 반영한 추천입니다.
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            {['15분', '마커콘', 'SPOMOVE 연결'].map((item) => (
              <span key={item} className="rounded-full bg-black/20 px-3 py-1 text-[11px] font-black text-white/75">
                {item}
              </span>
            ))}
          </div>
        </div>
        <span className="mt-5 grid h-12 w-12 place-items-center rounded-full md:mt-0" style={{ background: 'rgba(255,255,255,0.15)' }}>
    ...
    <section className="mb-7 grid grid-cols-3 gap-2 px-[22px] sm:px-8 lg:px-10">
      {items.map(([value, label]) => (
        <div key={label} className="rounded-[12px] p-3 text-center" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          <p className="text-[20px] font-black" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)' }}>
            {value}
          </p>
...
function ProgramRail({ title, caption, programs, isPro }: { title: string; caption: string; programs: typeof PROGRAMS; isPro: boolean }) {
  return (
    <section className="mb-7">
      <div className="mb-[14px] flex items-baseline justify-between px-[22px] sm:px-8 lg:px-10">
        <h2 className="text-[18px] font-bold" style={{ fontFamily: 'var(--spm-font-display)' }}>
          {title}
        </h2>
      ...
      <div className="flex gap-[9px] overflow-x-auto px-[22px] sm:px-8 lg:grid lg:grid-cols-5 lg:overflow-visible lg:px-10">
        {programs.map((program, index) => (
          <PosterCard key={`${title}-${program.id}`} program={program} rank={index + 1} locked={program.isPro && !isPro} />
        ))}
      </div>
    </section>
  );
    ...
    <div className="fixed inset-0 z-[80] flex justify-center bg-black/80">
      <div className="min-h-dvh w-full max-w-[1180px] px-[22px] pt-[22px] sm:px-8 lg:px-10" style={{ background: 'var(--spm-bg)' }}>
        ...
        <p className="mt-5 text-[12px] font-medium leading-6" style={{ color: 'var(--spm-t3)' }}>
          검색어를 입력하면 라이브러리 목록과 추천 레일에 바로 반영됩니다.
        </p>
      </div>
    </div>
  );
...
export default function LibraryView() {
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      ...
      <section className="mb-4 flex gap-2 overflow-x-auto px-[22px] sm:px-8 lg:px-10">
        {CATEGORIES.map((item) => (
          <Chip key={item} label={item} active={category === item} onClick={() => setCategory(item)} />
        ))}
      </section>
      <section className="mb-7 flex gap-2 overflow-x-auto px-[22px] sm:px-8 lg:px-10">
        {GRADES.map((item) => (
          <Chip key={item} label={item} active={grade === item} onClick={() => setGrade(item)} />
        ))}
      </section>

      <ProgramRail title="TOP 5" caption="이번 주 추천" programs={topPrograms} isPro={isPro} />
      <ProgramRail title="SPOMOVE 연결 수업" caption="웹 실행 포함" programs={spomovePrograms} isPro={isPro} />
      <ProgramRail title="15분 내외 빠른 수업" caption="대체 수업용" programs={quickPrograms} isPro={isPro} />
      ...
      <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
        <Link href="/spokedu-master/spomove" className="flex items-center gap-3 rounded-[14px] p-4" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-[12px]" style={{ background: 'rgba(99,102,241,0.14)' }}>
            <Zap size={19} color="var(--spm-acc)" />
          </span>
