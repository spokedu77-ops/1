...
function PosterCard({ program, rank, locked, used, favorite, onFavorite }: { program: Program; rank: number; locked: boolean; used: boolean; favorite: boolean; onFavorite: () => void }) {
  return (
    <div className="relative h-[196px] w-[140px] shrink-0 overflow-hidden rounded-[14px] lg:h-[210px] lg:w-full" style={{ background: `linear-gradient(160deg, ${program.colors[0]}, ${program.colors[1]}, ${program.colors[2]})` }}>
      <Link href={locked ? '/spokedu-master/payment?plan=pro' : `/spokedu-master/library/${program.id}`} className="absolute inset-0 active:scale-[0.98]">
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/82 to-transparent p-3">
          <div className="flex items-center justify-between gap-1">
            <p className="truncate text-[9px] font-bold uppercase tracking-[0.08em] text-white/45">{program.category}</p>
...
function ProgramListItem({ program, locked, used, favorite, onFavorite }: { program: Program; locked: boolean; used: boolean; favorite: boolean; onFavorite: () => void }) {
  return (
    <div className="relative flex h-full gap-3 rounded-[14px] p-3 active:scale-[0.99]" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br)' }}>
      <Link href={locked ? '/spokedu-master/payment?plan=pro' : `/spokedu-master/library/${program.id}`} className="absolute inset-0 rounded-[14px]" aria-label={program.title} />
...
function FeaturedRail({ programs }: { programs: Program[] }) {
  ...
  return (
    <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
      <Link href={`/spokedu-master/library/${featured.id}`} className="relative grid overflow-hidden rounded-[18px] p-5 active:scale-[0.99] md:grid-cols-[1fr_auto] md:items-end md:p-7" style={{ background: `linear-gradient(135deg, ${featured.colors[0]}, ${featured.colors[1]}, ${featured.colors[2]})`, boxShadow: '0 18px 42px rgba(99,102,241,0.2)' }}>
        <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 opacity-[0.08]" aria-hidden>
        ...
        <span className="relative mt-5 grid h-12 w-12 place-items-center rounded-full md:mt-0" style={{ background: 'rgba(255,255,255,0.15)' }}><Play size={18} color="#fff" fill="#fff" /></span>
      </Link>
    </section>
  );
}
  ...
  return (
    <section id={id} className="mb-7 scroll-mt-20">
      ...
      <div className="scrollbar-hide flex gap-[9px] overflow-x-auto px-[22px] sm:px-8 lg:grid lg:grid-cols-5 lg:overflow-visible lg:px-10">
        {programs.map((program, index) => <PosterCard key={`${title}-${program.id}`} program={program} rank={index + 1} locked={program.isPro && !isPro} used={usedProgramIds.has(program.id)} favorite={favorites.includes(program.id)} onFavorite={() => onFavorite(program.id)} />)}
      </div>
    </section>
  );
}
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      ...
      <section className="px-[22px] sm:px-8 lg:px-10">
        ...
        {filtered.length > 0
          ? <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{filtered.map((program) => <ProgramListItem key={program.id} program={program} locked={program.isPro && !isPro} used={usedProgramIds.has(program.id)} favorite={favorites.includes(program.id)} onFavorite={() => toggleFavorite(program.id)} />)}</div>
          : <div className="rounded-[14px] p-5 text-center" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}><p className="text-[14px] font-bold" style={{ color: 'var(--spm-t)' }}>조건에 맞는 수업안이 없습니다.</p><p className="mt-1 text-[12px]" style={{ color: 'var(--spm-t3)' }}>필터와 검색어를 조금 바꿔보세요.</p><button type="button" onClick={resetFilters} className="mt-4 h-10 rounded-[12px] px-4 text-[12px] font-black text-white" style={{ background: 'var(--spm-acc)' }}>전체 보기</button></div>
        }
      </section>
