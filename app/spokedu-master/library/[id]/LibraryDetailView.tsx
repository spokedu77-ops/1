...
import { ArrowLeft, CalendarPlus, Heart, Lightbulb, Lock, MessageCircle, Play, ShieldAlert, ShoppingBag, Shuffle, Zap } from 'lucide-react';
...
function DetailPanel({ title, icon: Icon, children }: { title: string; icon: typeof Lightbulb; children: React.ReactNode }) {
  return (
    ...
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      <header className="sticky top-0 z-20 flex items-center justify-between px-[22px] py-3 sm:px-8 lg:px-10" style={{ background: 'rgba(7,7,12,0.86)', backdropFilter: 'blur(18px)' }}>
        ...
        <div className="flex items-center gap-2">
          <Link href="/spokedu-master/shop" className="grid h-10 w-10 place-items-center rounded-[12px]" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }} aria-label="교구 샵">
      ...
      <main className="px-[22px] sm:px-8 lg:px-10">
        <section className="grid gap-5 md:grid-cols-[0.82fr_1.18fr]">
          ...
          <div className="pt-1 md:pt-4">
            ...
            {locked ? (
              <Link href="/spokedu-master/profile" className="mt-7 block rounded-[14px] p-4 text-center" style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.18)' }}>
                <strong className="block text-[14px]" style={{ color: 'var(--spm-amb)' }}>PRO로 업그레이드하고 전체 수업안 열기</strong>
              </Link>
            ) : (
              <div className="mt-7 grid grid-cols-1 gap-2 sm:grid-cols-2">
                <Link href="/spokedu-master/class-record" className="flex h-12 items-center justify-center gap-2 rounded-[12px] text-[14px] font-bold text-white" style={{ background: 'var(--spm-acc)', boxShadow: '0 8px 24px var(--spm-acc-glow)' }}>
                  <Play size={16} fill="#fff" />
                  수업 기록 시작
                </Link>
                <button type="button" onClick={() => setSheetOpen(true)} className="flex h-12 items-center justify-center gap-2 rounded-[12px] text-[14px] font-bold" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)', color: 'var(--spm-t)' }}>
        ...
        <section className="mt-7 rounded-[16px] p-4" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href="/spokedu-master/shop" className="text-[12px] font-bold" style={{ color: 'var(--spm-acc)' }}>교구 샵</Link>
          </div>
            ...
            {program.equipment.map((item) => {
              ...
              return (
                <div key={item} className="flex items-center gap-2 rounded-[12px] p-3" style={{ background: 'var(--spm-s3)' }}>
                  ...
                  {price > 0 ? (
                    <button type="button" onClick={() => addEquipment(item)} className="h-8 rounded-[10px] px-3 text-[11px] font-black text-white" style={{ background: 'var(--spm-acc)' }}>담기</button>
                  ) : null}
                </div>
              );
            })}
          ...
          <div className="mt-7 grid gap-4 lg:grid-cols-2">
            ...
            <DetailPanel title="연결 SPOMOVE" icon={Zap}>
              <div className="flex flex-wrap gap-2">
                {program.lessonDetail.relatedSpomoveIds.map((item) => (
                  <Link key={item} href={`/spokedu-master/spomove/session?drill=${item}&mode=class`} className="rounded-full px-3 py-2 text-[12px] font-black" style={{ background: 'rgba(99,102,241,0.14)', color: '#a5b4fc' }}>
                    {item}
                  </Link>
                ))}
              </div>
      ...
      <BottomSheet open={sheetOpen} title="수업 계획 추가" onClose={() => setSheetOpen(false)}>
        <div className="space-y-5">
          <div>
            <p className="mb-2 text-[12px] font-bold" style={{ color: 'var(--spm-t3)' }}>반 선택</p>
            <div className="flex flex-wrap gap-2">
              {['3학년 A반', '3학년 B반', '4학년 A반'].map((item) => <FieldButton key={item} label={item} active={classId === item} onClick={() => setClassId(item)} />)}
            </div>
          </div>
          <div>
            <p className="mb-2 text-[12px] font-bold" style={{ color: 'var(--spm-t3)' }}>교시</p>
            <div className="flex flex-wrap gap-2">
              {[1, 2, 3, 4, 5].map((item) => <FieldButton key={item} label={`${item}교시`} active={period === item} onClick={() => setPeriod(item)} />)}
            </div>
          </div>
          <label className="block">
            <span className="mb-2 block text-[12px] font-bold" style={{ color: 'var(--spm-t3)' }}>시간</span>
            <input type="number" min={5} max={60} value={duration} onChange={(event) => setDuration(Number(event.target.value))} className="h-11 w-full rounded-[12px] border bg-transparent px-3 text-[14px] font-bold outline-none" style={{ background: 'var(--spm-s2)', borderColor: 'var(--spm-br2)', color: 'var(--spm-t)' }} />
          </label>
