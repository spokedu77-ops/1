...
function getEquipmentPrice(item: string) {
  ...
  if (/마커|marker|콘|cone|접시콘/i.test(item)) return 8900;
  if (/카드|card/i.test(item)) return 12000;
  if (/밴드|band/i.test(item)) return 24000;
  if (/바통|풍선|baton|balloon|공|라바콘|펀스틱/i.test(item)) return 6900;
  return 0;
}
...
function PackageSummary({ program, drillName, reason }: { program: Program; drillName: string; reason: string }) {
  const items = [
  ...
  return (
    <section className="mt-7 rounded-[18px] p-4" style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.16), rgba(16,185,129,0.08), var(--spm-s2))', border: '1px solid rgba(99,102,241,0.26)' }}>
      ...
      <p className="mt-3 rounded-[13px] p-3 text-[12px] font-semibold leading-5" style={{ background: 'rgba(99,102,241,0.11)', color: '#c7d2fe' }}>
        {reason}
      </p>
    </section>
  );
...
export default function LibraryDetailView({ id }: { id: string }) {
  ...
  const primarySpomoveId = detail?.relatedSpomoveIds[0] ?? drills[0]?.id ?? 'SR-05';
  ...
  const spomoveReason = getSpomoveReason(program, primarySpomove);
  const metaItems = [
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      <header className="sticky top-0 z-20 flex items-center justify-between px-[22px] py-3 sm:px-8 lg:px-10" style={{ background: 'rgba(7,7,12,0.88)', backdropFilter: 'blur(18px)' }}>
      ...
      <section className="relative overflow-hidden" style={heroImage ? undefined : { background: `radial-gradient(ellipse 80% 60% at 70% 40%, ${program.colors[2]}55 0%, transparent 65%), linear-gradient(155deg, ${program.colors[0]}, ${program.colors[1]} 55%, ${program.colors[2]}cc)` }}>
        <div className="relative min-h-[360px] md:min-h-[480px]">
          ...
          {locked ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black/55">
              <span className="grid h-16 w-16 place-items-center rounded-[18px]" style={{ background: 'rgba(245,158,11,0.14)', border: '1px solid rgba(245,158,11,0.28)' }}>
          ...
          <div className="absolute inset-x-0 bottom-0 px-[22px] py-8 sm:px-8 lg:px-10">
            <div className="mb-3 flex flex-wrap gap-1.5">
              <span className="rounded-full px-2.5 py-1 text-[10px] font-black text-white/75" style={{ background: 'rgba(0,0,0,0.44)', border: '1px solid rgba(255,255,255,0.1)' }}>{program.category}</span>
              ...
              {usageCount > 0 ? (
                <span className="flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-black" style={{ background: 'rgba(16,185,129,0.22)', color: 'var(--spm-grn)', border: '1px solid rgba(16,185,129,0.28)' }}>
                  <CheckCircle2 size={9} />
                  {usageCount}회 사용{lastUsedRecord ? ` · ${new Date(lastUsedRecord.date).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric' })}` : ''}
                </span>
              ) : null}
            </div>
            <h1 className="max-w-[780px] text-[38px] font-black leading-[1.06] text-white sm:text-[54px] md:text-[62px]" style={{ fontFamily: 'var(--spm-font-display)', letterSpacing: 0, wordBreak: 'keep-all', textShadow: '0 2px 24px rgba(0,0,0,0.55)' }}>
              {program.title}
            </h1>
            <p className="mt-3 max-w-[620px] text-[13px] font-medium leading-6 text-white/60">{description}</p>
            <div className="mt-4 flex flex-wrap gap-2">
              {[
                ...
                <span key={label} className="flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[11px] font-bold text-white/80" style={{ background: 'rgba(0,0,0,0.32)', border: '1px solid rgba(255,255,255,0.1)' }}>
                  <Icon size={11} />{label}
                </span>
              ))}
            </div>
      ...
      <main className="px-[22px] pb-7 pt-6 sm:px-8 lg:px-10">
        <section>
          {locked ? (
            <Link href="/spokedu-master/payment?plan=pro" className="flex h-[54px] w-full items-center justify-center gap-2 rounded-[16px] text-[15px] font-black text-white" style={{ background: 'var(--spm-acc)', boxShadow: '0 10px 32px rgba(99,102,241,0.38)' }}>
              <Zap size={17} />Pro로 전체 수업안 보기
            </Link>
          ) : (
            <div className="flex flex-col gap-2">
              <Link href={`/spokedu-master/class-mode/${program.id}`} className="flex h-[54px] w-full items-center justify-center gap-2 rounded-[16px] text-[15px] font-black text-white" style={{ background: 'var(--spm-acc)', boxShadow: '0 10px 32px rgba(99,102,241,0.35)' }}>
                <Play size={17} fill="#fff" />수업 시작
              </Link>
              <div className="grid grid-cols-3 gap-2">
                <Link href={`/spokedu-master/spomove/session?drill=${primarySpomoveId}&mode=projector&program=${program.id}`} className="flex h-11 items-center justify-center gap-1.5 rounded-[13px] text-[12px] font-bold" style={{ background: 'linear-gradient(135deg,rgba(99,102,241,0.14),rgba(16,185,129,0.08))', border: '1px solid rgba(99,102,241,0.22)', color: '#a5b4fc' }}>
                  <MonitorPlay size={14} />SPOMOVE
                </Link>
                {detail?.videoUrl ? (
                  <a href={detail.videoUrl} target="_blank" rel="noopener noreferrer" className="flex h-11 items-center justify-center gap-1.5 rounded-[13px] text-[12px] font-bold" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)', color: 'var(--spm-t)' }}>
                    <Play size={13} />영상
                  </a>
                ) : (
                  <Link href={`/spokedu-master/report?program=${program.id}`} className="flex h-11 items-center justify-center gap-1.5 rounded-[13px] text-[12px] font-bold" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)', color: 'var(--spm-t)' }}>
                    <MessageCircle size={13} />설명 문구
                  </Link>
                )}
                <button type="button" onClick={copyParentNote} className="flex h-11 items-center justify-center gap-1.5 rounded-[13px] text-[12px] font-bold" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)', color: copied ? 'var(--spm-grn)' : 'var(--spm-t)' }}>
        ...
        <PackageSummary program={program} drillName={primarySpomove?.name ?? 'SPOMOVE 큰 화면'} reason={spomoveReason} />

        {galleryImages.length > 0 ? (
          <section className="mt-7">
            <div className="mb-3 flex items-center gap-2">
              <ImageIcon size={16} color="var(--spm-acc)" />
              <h2 className="text-[18px] font-bold" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)' }}>현장 이미지</h2>
            </div>
            <div className="scrollbar-hide flex gap-2 overflow-x-auto">
              {galleryImages.map((src) => (
                <div key={src} className="h-[180px] w-[280px] shrink-0 overflow-hidden rounded-[16px]" style={{ border: '1px solid var(--spm-br2)' }}>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        {ruleItems.length > 0 ? (
          <section className="mt-7">
            <h2 className="mb-4 text-[18px] font-bold" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)' }}>How to Play · {ruleItems.length}단계</h2>
            <ol className="space-y-2">
              {ruleItems.map((step, index) => (
                <li key={`${index}-${step}`} className="flex gap-3 rounded-[14px] p-4" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br)' }}>
                  <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full text-[11px] font-black text-white" style={{ background: 'var(--spm-acc)' }}>{index + 1}</span>
          ...
          <section className="mt-7 rounded-[16px] p-4" style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.12), rgba(16,185,129,0.06))', border: '1px solid rgba(99,102,241,0.2)' }}>
            ...
            <div className="grid gap-2">
              {relatedSpomoveIds.map((item) => {
                ...
                return (
                  <Link key={item} href={`/spokedu-master/spomove/session?drill=${item}&mode=class&program=${program.id}`} className="flex items-center gap-3 rounded-[13px] p-3 active:scale-[0.99]" style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.18)' }}>
        ...
        {detail ? (
          <div className="mt-4 grid gap-4 lg:grid-cols-2">
            {detail.setupNotes && detail.setupNotes.length > 0 ? <DetailPanel title="세팅 노트" icon={ImageIcon}><DetailList items={detail.setupNotes} /></DetailPanel> : null}
