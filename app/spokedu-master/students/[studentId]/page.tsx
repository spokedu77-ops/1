...
export default function StudentDetailPage() {
  ...
  const recentObservations = entries
    .filter((entry) => entry.student.memo?.trim() || entry.student.skills.length)
    .slice(0, 2);
    ...
    return (
      <div className="h-full overflow-y-auto p-[22px] pb-28 lg:p-10" style={{ background: 'var(--spm-bg)' }}>
        <section className="rounded-[18px] p-6 text-center" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          ...
          <Link href="/spokedu-master/students" className="mt-5 inline-flex min-h-11 items-center justify-center rounded-[12px] px-5 text-[13px] font-black text-white" style={{ background: 'var(--spm-acc)' }}>학생 목록으로</Link>
        </section>
      </div>
    );
  }
      ...
      <main className="grid gap-5 px-[22px] sm:px-8 lg:px-10">
        ...
        <section className="rounded-[18px] p-5" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
            ...
            {latest ? <Info label="최근 프로그램" value={latest.record.programTitle} /> : null}
          </div>
        </section>

        <section className="rounded-[18px] p-5" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          <h2 className="flex items-center gap-2 text-[18px] font-black" style={{ color: 'var(--spm-t)' }}><CalendarDays size={17} color="var(--spm-acc)" />4. 최근 수업 기록</h2>
          {entries.length ? (
            <div className="mt-4 grid gap-3">
              {entries.slice(0, 5).map(({ record, student: recordStudent }) => (
                <article key={record.id} className="rounded-[14px] p-4" style={{ background: 'var(--spm-s3)', border: '1px solid var(--spm-br2)' }}>
                  <p className="text-[12px] font-bold" style={{ color: 'var(--spm-t3)' }}>{formatDate(record.date)} · {getAttendanceLabel(recordStudent.attendance)}</p>
                  <h3 className="mt-1 break-words text-[16px] font-black" style={{ color: 'var(--spm-t)' }}>{record.programTitle}</h3>
                  {record.memo ? <p className="mt-2 break-words rounded-[10px] p-2 text-[12px] font-semibold leading-5" style={{ background: 'var(--spm-s2)', color: 'var(--spm-t2)' }}>공통 메모: {record.memo}</p> : null}
                  {recordStudent.skills.length ? <p className="mt-2 text-[12px] font-semibold" style={{ color: 'var(--spm-t2)' }}>수행 기록: {recordStudent.skills.join(', ')}</p> : null}
                  {recordStudent.memo ? <p className="mt-2 break-words rounded-[10px] p-2 text-[12px] font-semibold leading-5" style={{ background: 'var(--spm-s2)', color: 'var(--spm-t)' }}>학생별 메모: {recordStudent.memo}</p> : null}
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Link href={`/spokedu-master/class-record?record=${record.id}&student=${student.id}`} className="inline-flex min-h-10 items-center rounded-[10px] px-3 text-[11px] font-black" style={{ background: 'rgba(99,102,241,0.12)', color: 'var(--spm-acc)' }}>기록 보기·수정</Link>
                    <Link href={`/spokedu-master/report?record=${record.id}`} className="inline-flex min-h-10 items-center rounded-[10px] px-3 text-[11px] font-black" style={{ background: 'rgba(16,185,129,0.12)', color: 'var(--spm-grn)' }}>안내문 작성</Link>
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="mt-3 grid gap-2">
              {recentObservations.map(({ record, student: recordStudent }) => (
                <p key={record.id} className="break-words rounded-[12px] p-3 text-[12px] font-semibold leading-5" style={{ background: 'var(--spm-s3)', color: 'var(--spm-t2)' }}>
                  {formatDate(record.date)} · {[recordStudent.skills.join(', '), recordStudent.memo].filter(Boolean).join(' / ')}
                </p>
              ))}
            </div>
          </section>
