      ...
      <main className="mt-4 grid gap-4">
        ...
        <section className="rounded-[16px] border border-slate-200 bg-white/86 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-3">
            ...
            {latest ? (
              ...
              <span className="flex h-9 items-center justify-center gap-2 rounded-[9px] border border-slate-200 bg-white px-3 text-[12px] font-black text-slate-500 opacity-60"><FileText size={15} />기록 후 안내문</span>
            )}
            <Link href="/spokedu-master/students" className="flex h-9 items-center justify-center gap-2 rounded-[9px] border border-slate-200 bg-white px-3 text-[12px] font-black text-slate-700"><Users size={15} />학생 목록 관리</Link>
          </div>
        </section>
        ...
        <section className="rounded-[16px] border border-slate-200 bg-white/86 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
          <h2 className="flex items-center gap-2 text-[18px] font-black" style={{ color: 'var(--spm-t)' }}>
            <CalendarDays size={17} className="text-slate-700" />
            4. 최근 수업 기록
          </h2>
          ...
          {entries.length ? (
            <div className="mt-4 grid gap-3">
              {entries.slice(0, 5).map(({ record, student: recordStudent }) => {
                ...
                return (
                  <article key={record.id} className="rounded-[14px] border border-slate-200 bg-white p-3 shadow-[0_8px_18px_rgba(15,23,42,0.04)]">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <Link href={`/spokedu-master/report?record=${record.id}`} className="inline-flex h-9 items-center rounded-[9px] border border-slate-200 bg-white px-3 text-[11px] font-black text-slate-700">
                        안내문 작성
                      </Link>
                    </div>
                  </article>
            ...
            <div className="mt-4 rounded-[14px] border border-dashed border-slate-200 bg-slate-50 p-4 text-slate-600">
          ...
          <section className="rounded-[16px] border border-slate-200 bg-white/86 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
            ...
            <div className="mt-3 grid gap-2">
              {recentObservations.map(({ record, student: recordStudent }) => {
                ...
                return (
                  <p key={record.id} className="break-words rounded-[12px] border border-slate-200 bg-slate-50 p-3 text-[12px] font-semibold leading-5 text-slate-600">
        ...
        <section className="rounded-[16px] border border-slate-200 bg-white/86 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.05)]">
          ...
          <div className="mt-3 rounded-[14px] border border-slate-200 bg-slate-50 p-4 text-slate-600">
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[12px] border border-slate-200 bg-slate-50 p-3">
      ...
      <p className="mt-1 break-words text-[13px] font-black text-slate-950">{value}</p>
    </div>
  );
}
