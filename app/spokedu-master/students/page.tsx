...
export default function StudentsPage() {
  ...
  return <main className="h-full overflow-y-auto bg-[var(--spm-bg)] pb-28 lg:pb-8"><div className="mx-auto max-w-5xl px-4 py-5 sm:px-6">
    <header className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black text-slate-500">명단 관리</p><h1 className="mt-1 text-2xl font-black text-slate-900">학생</h1><p className="mt-2 text-sm font-semibold text-slate-500">학생을 수업반에 등록하면 수업의 출석 명단으로 자동 연결됩니다.</p></div><button type="button" onClick={() => { setDraft(EMPTY_DRAFT); setError(null); setAddOpen(true); }} className={SPM_PRIMARY_BTN}><Plus size={16} />{MASTER_ACTION_COPY.addStudent}</button></header>
    ...
    <section className="mt-5 grid gap-3 sm:grid-cols-2">{data.students.map((student) => <article key={student.id} className={SPM_COLLECTION_CARD}><div className={SPM_COLLECTION_CARD_BODY}><div className="flex items-center gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-slate-900 font-black text-white">{student.name.slice(0, 1)}</span><div className="min-w-0 flex-1"><h2 className="truncate text-base font-black text-slate-900" title={student.name}>{student.name}</h2><p className="mt-1 truncate text-xs font-semibold text-slate-500">{[classNames(student.id).join(', '), studentMetaToDisplay(student.meta)].filter(Boolean).join(' · ') || '수업반 미지정'}</p></div><span className="shrink-0 text-xs font-black text-slate-400">수업 {sessionCount(student.id)}건</span></div></div><div className={`${SPM_COLLECTION_CARD_FOOTER} sm:grid-cols-[minmax(0,1fr)_auto_auto]`}><Link href={`/spokedu-master/students/${student.id}`} className={SPM_PRIMARY_BTN}>학생 기록 보기<ChevronRight size={14} /></Link><button type="button" onClick={() => openEdit(student)} className={SPM_SECONDARY_BTN}><Pencil size={14} />학생 정보 수정</button><details className="relative"><summary aria-label={`${student.name} 학생 메뉴`} className="grid h-11 w-11 cursor-pointer list-none place-items-center rounded-xl border border-slate-200 text-slate-500"><MoreHorizontal size={18} /></summary><div className="absolute right-0 z-30 mt-1 w-40 rounded-xl border border-slate-200 bg-white p-1 shadow-lg"><button type="button" onClick={() => setPendingArchive(student)} disabled={deletingId === student.id} className="flex min-h-11 w-full items-center rounded-lg px-3 text-left text-xs font-black text-rose-600 disabled:opacity-40">{MASTER_ACTION_COPY.archiveStudent}</button></div></details></div></article>)}</section>
    {data.status === 'ready' && !data.students.length ? <MasterStatePanel kind="empty" title="아직 등록된 학생이 없습니다." description="학생을 추가하고 수업반을 선택해 주세요." icon={<Users size={24} />} action={<button type="button" onClick={() => { setDraft(EMPTY_DRAFT); setError(null); setAddOpen(true); }} className={SPM_PRIMARY_BTN}><Plus size={16} />{MASTER_ACTION_COPY.addStudent}</button>} className="mt-5" /> : null}
  </div>
  ...
  {pendingArchive ? <BottomSheet open title={MASTER_ACTION_COPY.archiveStudent} onClose={() => setPendingArchive(null)}><div className="space-y-4 pb-3"><p className="text-sm font-semibold leading-6 text-slate-600"><strong className="text-slate-900">{pendingArchive.name}</strong> 학생을 현재 명단에서 숨깁니다. 과거 수업과 출석 이력은 그대로 보존됩니다.</p>{error ? <p className="rounded-xl bg-rose-50 p-3 text-xs font-bold text-rose-700">{error}</p> : null}<button type="button" disabled={Boolean(deletingId)} onClick={() => void removeStudent(pendingArchive)} className={SPM_DESTRUCTIVE_BTN}>{deletingId ? '보관 중…' : MASTER_ACTION_COPY.archiveStudent}</button><button type="button" disabled={Boolean(deletingId)} onClick={() => setPendingArchive(null)} className="min-h-11 w-full text-sm font-black text-slate-600">돌아가기</button></div></BottomSheet> : null}
  </main>;
}
