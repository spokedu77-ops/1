...
import { Bell, BookOpen, ChevronRight, Lock, Play, Smartphone, Sparkles, UsersRound, Zap } from 'lucide-react';
...
function ProgramCard({ program, used }: { program: Program; used: boolean }) {
  return (
    <Link href={`/spokedu-master/library/${program.id}`} className="group flex min-h-[174px] flex-col justify-between rounded-[16px] p-4 transition-transform active:scale-[0.98]" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex flex-wrap gap-1.5">
            ...
            {program.isHot ? <span className="rounded-full px-2 py-1 text-[9px] font-black" style={{ background: 'rgba(245,158,11,0.14)', color: 'var(--spm-amb)' }}>HOT</span> : null}
            {used ? <span className="rounded-full px-2 py-1 text-[9px] font-black" style={{ background: 'rgba(99,102,241,0.14)', color: '#a5b4fc' }}>사용 이력</span> : null}
          </div>
          <h3 className="mt-3 text-[18px] font-black leading-tight" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)', letterSpacing: 0, wordBreak: 'keep-all' }}>{program.title}</h3>
          <p className="mt-2 line-clamp-2 text-[12px] font-medium leading-5" style={{ color: 'var(--spm-t2)' }}>{program.description}</p>
        </div>
        <ThumbGrid colors={program.colors} size={58} />
      </div>
      <div className="mt-4 flex items-center justify-between gap-2">
        <span className="text-[11px] font-bold" style={{ color: 'var(--spm-t3)' }}>{program.grade} · {program.duration}분</span>
  ...
  return (
    <Link href="/spokedu-master/profile" className="mx-[22px] mb-6 flex items-center gap-2.5 rounded-[10px] px-[14px] py-[11px] sm:mx-8 lg:mx-10" style={{ background: 'linear-gradient(90deg, rgba(245,158,11,0.08), rgba(245,158,11,0.04))', border: '1px solid rgba(245,158,11,0.18)' }}>
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="mt-0.5 block text-[10px]" style={{ color: 'rgba(245,158,11,0.65)' }}>전체 프로그램, SPOMOVE 실행, 성장 리포트 기능을 엽니다.</span>
      </span>
    ...
    <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
      <div className="grid gap-5 overflow-hidden rounded-[18px] p-5 md:grid-cols-[1.35fr_0.65fr] md:p-7" style={{ background: 'linear-gradient(135deg, rgba(24,95,165,0.28), rgba(29,158,117,0.14), var(--spm-s2))', border: '1px solid rgba(99,102,241,0.28)' }}>
        <div>
          ...
          <h2 className="mt-3 max-w-[680px] text-[26px] font-black leading-[1.16] md:text-[34px]" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)', letterSpacing: 0, wordBreak: 'keep-all' }}>놀이체육과 SPOMOVE를 구독하는 프로그램 라이브러리</h2>
          <p className="mt-3 max-w-[680px] text-[13px] font-medium leading-6 md:text-[15px]" style={{ color: 'var(--spm-t2)' }}>수업안은 넷플릭스처럼 고르고, SPOMOVE는 웹에서 바로 실행하고, 수업 기록은 학생 성장 이력과 보호자 리포트로 쌓입니다.</p>
        </div>
        <div className="grid grid-cols-2 gap-2 md:grid-cols-1">
          <Link href="/spokedu-master/library" className="flex h-12 items-center justify-center gap-2 rounded-[12px] text-[13px] font-black text-white" style={{ background: 'var(--spm-acc)' }}><BookOpen size={15} />프로그램 탐색</Link>
    ...
    <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
      <div className="relative overflow-hidden rounded-[14px]" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
        ...
        <div className="grid gap-4 p-[18px] md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <span className="mb-3 inline-flex items-center gap-[5px] rounded-[6px] px-[9px] py-[3px] text-[10px] font-bold" style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', color: 'var(--spm-grn)' }}><span className="h-[5px] w-[5px] animate-pulse rounded-full" style={{ background: 'var(--spm-grn)' }} />{todayLesson?.classId ?? '초등 A반'} 진행 예정</span>
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
          ...
          <div className="grid grid-cols-[1fr_48px] gap-2 md:min-w-[260px]">
            <Link href={`/spokedu-master/class-record?program=${program.id}`} className="flex items-center justify-center gap-2 rounded-[10px] px-[18px] py-[13px] text-[14px] font-semibold text-white active:scale-[0.98]" style={{ background: 'var(--spm-acc)', boxShadow: '0 8px 24px var(--spm-acc-glow)', fontFamily: 'var(--spm-font-display)' }}><Play size={14} fill="white" />수업 기록하기</Link>
            <Link href={`/spokedu-master/library/${program.id}`} className="grid place-items-center rounded-[10px]" style={{ background: 'var(--spm-s3)', border: '1px solid var(--spm-br2)' }} aria-label="수업 상세 보기"><ChevronRight size={17} color="var(--spm-t2)" /></Link>
          </div>
        </div>
      </div>
    </section>
...
function ServiceHealthStrip() {
  ...
  return (
    <section className="mb-7 grid gap-2 px-[22px] sm:grid-cols-3 sm:px-8 lg:px-10">
      <Link href="/spokedu-master/profile" className="rounded-[12px] p-3" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}><p className="text-[10px] font-bold" style={{ color: 'var(--spm-t3)' }}>구독 상태</p><p className="mt-1 text-[14px] font-black" style={{ color: 'var(--spm-t)' }}>{planLabel}</p></Link>
      <Link href="/spokedu-master/profile" className="rounded-[12px] p-3" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}><p className="text-[10px] font-bold" style={{ color: 'var(--spm-t3)' }}>동기화</p><p className="mt-1 text-[14px] font-black" style={{ color: operational.online ? 'var(--spm-grn)' : 'var(--spm-amb)' }}>{operational.online ? '온라인 정상' : '오프라인 저장'}</p></Link>
      <Link href="/spokedu-master/profile" className="rounded-[12px] p-3" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}><p className="text-[10px] font-bold" style={{ color: 'var(--spm-t3)' }}>재시도</p><p className="mt-1 text-[14px] font-black" style={{ color: operational.retryQueue.length ? 'var(--spm-amb)' : 'var(--spm-t)' }}>{operational.retryQueue.length}건 대기</p></Link>
    </section>
  );
}
  ...
  return (
    <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
      <div className="rounded-[18px] p-5" style={{ background: 'linear-gradient(135deg, rgba(29,158,117,0.16), var(--spm-s2))', border: '1px solid rgba(16,185,129,0.24)' }}>
        ...
        <p className="mt-2 text-[13px] font-medium leading-6" style={{ color: 'var(--spm-t2)' }}>강사 기록률, 이탈 위험 학생, 센터 플랜 사용량을 기준으로 운영 리스크를 조기 확인합니다.</p>
...
export default function DashboardView() {
  ...
  const featuredPrograms = useMemo(() => PROGRAMS.slice(0, 4), []);
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      ...
      <section className="mb-7 grid grid-cols-2 gap-2 px-[22px] sm:grid-cols-4 sm:px-8 lg:px-10">
        <KpiCard value={String(PROGRAMS.length)} label="구독 프로그램" delta="+8 예정" />
        <KpiCard value={String(activeClasses || 3)} label={profile?.role === 'director' ? '운영 반' : '진행 반'} delta="오늘 2개" />
        <KpiCard value={String(classRecords.length)} label="누적 기록" delta="학생 이력화" />
      ...
      <SectionHeader title="추천 라이브러리" href="/spokedu-master/library" />
      ...
      <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
        <Link href="/spokedu-master/students" className="flex items-center gap-3 rounded-[14px] p-4" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          ...
          <span className="min-w-0 flex-1"><strong className="block text-[14px]" style={{ color: 'var(--spm-t)' }}>학생 성장 이력</strong><span className="mt-1 block text-[11px]" style={{ color: 'var(--spm-t3)' }}>수업 기록이 배지, 위험 신호, 보호자 리포트로 누적됩니다.</span></span>
      ...
      <section className="mb-7 px-[22px] sm:px-8 lg:px-10">
        <div className="flex items-center gap-3 rounded-[14px] p-4" style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.16), rgba(16,185,129,0.1))', border: '1px solid var(--spm-br2)' }}>
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-[12px]" style={{ background: 'rgba(255,255,255,0.08)' }}><Sparkles size={19} color="#a5b4fc" /></span>
          <span className="min-w-0 flex-1"><strong className="block text-[14px]" style={{ color: 'var(--spm-t)' }}>다음 상용화 과제</strong><span className="mt-1 block text-[11px] leading-5" style={{ color: 'var(--spm-t3)' }}>플레이리스트, 수업 기록 상세 수정, 보호자 공유 링크의 신뢰도를 순서대로 강화합니다.</span></span>
        </div>
      </section>
