...
export function CompactOpsBar({
  ...
  const hasUnfinishedDraft = anchor.kind === 'record_draft' || anchor.kind === 'report_draft';
  const countsLabel =
    recordCount === null
      ? '기록 확인 중'
      : reportCount === null
  ...
  if (todayLessons.length > 0) {
    return (
      <section
        ...
        aria-label="이어갈 작업과 오늘 수업"
        className="grid gap-3 rounded-[14px] border border-slate-300 bg-white px-3 py-3 shadow-[0_8px_18px_rgba(15,23,42,0.07)] sm:px-4"
      >
        {hasUnfinishedDraft ? (
          <div data-compact-ops-section="unfinished" className="flex min-w-0 items-center gap-3 rounded-[11px] border border-slate-200 bg-slate-50 px-3 py-2">
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">이어갈 작업</p>
              <p className="mt-0.5 truncate text-[14px] font-black leading-tight text-[color:var(--spm-t)]">{anchor.title}</p>
            </div>
            <Link
        ...
        <div data-compact-ops-section="today-lessons">
