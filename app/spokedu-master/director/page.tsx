  ...
  return (
    <div className="h-full overflow-y-auto pb-28 lg:pb-7" style={{ background: 'var(--spm-bg)' }}>
      <header className="px-[22px] pb-5 pt-[22px] sm:px-8 lg:px-10">
        <p className="text-[12px] font-semibold uppercase tracking-[0.14em]" style={{ color: 'var(--spm-t3)' }}>센터 운영</p>
      ...
      <section className="grid gap-3 px-[22px] sm:grid-cols-2 sm:px-8 lg:grid-cols-4 lg:px-10">
        <Kpi label="기록" value={`${recordFacts.recordCount}건`} desc="전체 수업 기록 수" icon={BarChart3} tone="var(--spm-acc-muted)" />
        <Kpi label="출석" value={`${recordFacts.presentCount}건`} desc="출석으로 체크된 기록" icon={UserCheck} tone="var(--spm-grn)" />
        <Kpi label="결석" value={`${recordFacts.absentCount}건`} desc="결석으로 체크된 기록" icon={UserX} tone="var(--spm-amb)" />
        <Kpi label="학생" value={`${recordFacts.recordedStudentCount}명`} desc="기록이 연결된 학생" icon={UsersRound} tone="var(--spm-acc)" />
      </section>
      ...
      <div className="mt-7 grid gap-5 px-[22px] sm:px-8 lg:grid-cols-[1fr_380px] lg:px-10">
        ...
        <section className="rounded-[18px] p-5" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          ...
          <div className="mt-5 space-y-2">
            <p className="rounded-[13px] p-3 text-[12px] font-bold" style={{ background: 'var(--spm-grn-a10)', color: 'var(--spm-grn)' }}>
      ...
      <section className="mx-[22px] mt-5 grid gap-3 sm:mx-8 md:grid-cols-3 lg:mx-10">
        <Link href="/spokedu-master/report" className="flex items-center gap-3 rounded-[16px] p-4" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          <span className="grid h-11 w-11 place-items-center rounded-[12px]" style={{ background: 'var(--spm-acc-a14)' }}><FileText size={19} color="var(--spm-acc)" /></span>
        ...
        <Link href="/spokedu-master/class-record" className="flex items-center gap-3 rounded-[16px] p-4" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          <span className="grid h-11 w-11 place-items-center rounded-[12px]" style={{ background: 'var(--spm-grn-a14)' }}><MessageCircle size={19} color="var(--spm-grn)" /></span>
        ...
        <Link href="/spokedu-master/profile" className="flex items-center gap-3 rounded-[16px] p-4" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          <span className="grid h-11 w-11 place-items-center rounded-[12px]" style={{ background: 'var(--spm-amb-a14)' }}><CreditCard size={19} color="var(--spm-amb)" /></span>
      ...
      {profile?.plan === 'team' ? (
        <section className="mx-[22px] mt-5 rounded-[18px] p-5 sm:mx-8 lg:mx-10" style={{ background: 'linear-gradient(135deg, var(--spm-acc-a16), var(--spm-s2))', border: '1px solid var(--spm-acc-a28)' }}>
          <p className="text-[10px] font-black uppercase tracking-[0.14em]" style={{ color: 'var(--spm-acc-muted)' }}>센터 플랜</p>
        ...
        <section className="mx-[22px] mt-5 rounded-[18px] p-5 sm:mx-8 lg:mx-10" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-acc-a28)' }}>
          <p className="text-[10px] font-black uppercase tracking-[0.14em]" style={{ color: 'var(--spm-acc-muted)' }}>센터 플랜</p>
