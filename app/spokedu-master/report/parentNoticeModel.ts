...
export function buildDefaultParentNotice(session: MasterSessionDto) {
  ...
  return [
    ...
    completedPrograms.length ? `오늘은 ${completedPrograms.map((item) => item.sourceType === 'spomove' ? resolveSpomovePublicDisplayTitle(item.spomovePresetId, item.programTitle) : item.programTitle ?? '이름 없는 활동').join(', ')} 활동을 진행했습니다.` : '오늘 수업은 활동 완료 기록 없이 진행되었습니다.',
