...
function DrillCard({ drill, index, isLocked }: { drill: (typeof DRILLS)[number]; index: number; isLocked: boolean }) {
  ...
  return (
    <Link href={isLocked ? '/spokedu-master/profile' : `/spokedu-master/spomove/session?drill=${drill.id}`} className="relative min-h-[148px] overflow-hidden rounded-[14px] p-4 active:scale-[0.98]" style={{ background: gradients[index % gradients.length], border: '1px solid rgba(255,255,255,0.07)' }}>
