...
describe(`OFFICIAL_SPOMOVE_LIBRARY ${OFFICIAL_SPOMOVE_LIBRARY_SIZE}개 확장 계약`, () => {
  ...
  it('그룹별 개수가 확장 목표와 일치한다', () => {
    expect(byGroup('reaction-cognition')).toHaveLength(45);
    expect(byGroup('visual-reaction')).toHaveLength(19);
    ...
    expect(byGroup('dive')).toHaveLength(7);
    expect(byGroup('bonus')).toHaveLength(1);
  });

  it('DIVE 2단계 색 포즈 관문 공식 프리셋이 level 2 colorGate로 매핑된다', () => {
    const colorGate = byGroup('dive').find((preset) => preset.id === 'dive-color-gate-61');
    expect(colorGate).toBeTruthy();
    expect(colorGate!.engine.mode).toBe('flow');
  ...
  it('숫자 기차 tier 1/2/3 · 흰 공 tier 1/2/3 · 두더지 잡기가 존재한다', () => {
    ...
    expect(vr.some((preset) => preset.engine.mode === 'reactTrain' && preset.engine.level === 7)).toBe(true);
    expect(vr.some((preset) => preset.engine.level === 4 && preset.engine.camouflagePlacement === 'variant')).toBe(true);
    expect(vr.some((preset) => preset.engine.level === 7 && preset.engine.moleLookMode === 'variant')).toBe(true);
    expect(vr.some((preset) => preset.engine.level === 10 && preset.engine.colorTrackerDualPanel === true)).toBe(true);
  });

  it('DIVE 1단계 선택 장애물 punch/kick/duck/reach가 모두 공식 프리셋에 있다', () => {
    const diveFeatures = byGroup('dive').flatMap((preset) => preset.engine.flowFeatures ?? []);
    for (const feature of ['punch', 'kick', 'duck', 'reach'] as const) {
      expect(diveFeatures).toContain(feature);
    }
  });
