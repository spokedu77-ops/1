...
describe(`OFFICIAL_SPOMOVE_LIBRARY ${OFFICIAL_SPOMOVE_LIBRARY_SIZE}개 확장 계약`, () => {
  ...
  it('숫자 기차 tier 1/2/3 · 흰 공 tier 1/2/3 · 두더지 L1/L2가 존재한다', () => {
    ...
    expect(findOfficialSpomovePreset('visual-reaction-mole-l2')?.engine.moleLookMode).toBe('variant');

    expect(findOfficialSpomovePreset('visual-reaction-blackout-37')?.engine.camouflagePlacement).toBe('center');
    expect(findOfficialSpomovePreset('visual-reaction-camouflage-l2')?.engine.camouflagePlacement).toBe('variant');
