...
function getModeConfig(launchMode: string) {
  if (launchMode === 'class') {
    return {
      ...
      doneTitle: '수업 실행 완료',
      doneDescription: 'SPOMOVE 실행이 끝났습니다. 이어서 수업 기록을 남기거나 보호자 안내 문구로 정리할 수 있습니다.',
    };
  }
...
function SpomoveSessionContent() {
  ...
  return (
    <div className="relative h-dvh overflow-hidden select-none" style={{ background: state === 'running' ? currentCue.bgColor : '#050509', color: '#fff', fontFamily: 'var(--spm-font-display)', transition: 'background 0.12s ease' }} onClick={state === 'running' ? handleResponse : undefined} role={state === 'running' ? 'button' : undefined} tabIndex={state === 'running' ? 0 : undefined}>
      ...
      {state === 'running' ? <><div className="flex h-full flex-col items-center justify-center gap-4 px-8 text-center"><span key={cueSerial} className="animate-[spmCuePop_0.22s_cubic-bezier(.34,1.56,.64,1)_both] text-[clamp(92px,26vw,230px)] font-black leading-none text-white drop-shadow-[0_18px_50px_rgba(0,0,0,0.28)]">{currentCue.symbol}</span><span className="text-[18px] font-semibold uppercase tracking-[0.12em] text-white/55 sm:text-[22px]">{currentCue.label}</span></div>{modeConfig.showReactionBadge && lastRT !== null ? <div key={lastRT} className="absolute right-5 top-5 z-40 rounded-full px-3 py-1 text-[14px] font-black shadow-xl" style={{ background: 'rgba(0,0,0,0.35)', color: reactionColor(lastRT) }}>{lastRT}ms</div> : null}{modeConfig.showMetricsDuringRun ? <div className="absolute inset-x-0 bottom-0 z-30 grid grid-cols-3 gap-2 bg-gradient-to-t from-black/70 to-transparent px-5 pt-12" style={{ paddingBottom: 'max(28px, env(safe-area-inset-bottom))' }}><ResultStat label="횟수" value={`${recordedCount}/${maxCues}`} /><ResultStat label="평균" value={avg ? `${avg}ms` : '-'} tone="var(--spm-grn)" /><ResultStat label="최고" value={best ? `${best}ms` : '-'} /></div> : <div className="absolute inset-x-0 bottom-0 z-30 h-1 bg-white/10"><span className="block h-full transition-all duration-200" style={{ width: `${Math.min(100, (recordedCount / maxCues) * 100)}%`, background: 'rgba(255,255,255,0.72)' }} /></div>}</> : null}
      ...
      {state === 'done' ? <div className="flex h-full flex-col items-center justify-center px-8 text-center"><div className="grid h-[92px] w-[92px] place-items-center rounded-full animate-[spmCuePop_0.28s_cubic-bezier(.34,1.56,.64,1)_both]" style={{ background: 'rgba(255,255,255,0.1)' }}><Check size={42} color="var(--spm-grn)" /></div><h1 className="mt-6 text-[34px] font-black">{modeConfig.doneTitle}</h1>{program ? <p className="mt-2 text-[12px] font-black uppercase tracking-[0.12em] text-indigo-200/70">{program.title}</p> : null}<p className="mt-2 max-w-[460px] text-[13px] font-semibold leading-6 text-white/45">{modeConfig.doneDescription}</p><div className="mt-7 grid w-full max-w-[560px] grid-cols-3 gap-2"><ResultStat label="횟수" value={String(finalSession?.cueCount ?? recordedCount)} /><ResultStat label="평균" value={avg ? `${avg}ms` : '-'} tone="var(--spm-grn)" /><ResultStat label="최고" value={best ? `${best}ms` : '-'} /></div><div className={`mt-7 grid w-full max-w-[640px] gap-2 ${programId ? 'grid-cols-2 sm:grid-cols-4' : 'grid-cols-2'}`}><button type="button" onClick={startCountdown} className="flex h-12 items-center justify-center gap-2 rounded-[12px] text-[14px] font-bold text-white" style={{ background: 'var(--spm-acc)' }}><RotateCcw size={16} />다시 시작</button>{programId ? <Link href={`/spokedu-master/class-record?program=${programId}`} className="flex h-12 items-center justify-center gap-1.5 rounded-[12px] text-[13px] font-bold" style={{ background: 'rgba(255,255,255,0.08)', color: 'var(--spm-t)' }}><ClipboardList size={14} />수업 기록</Link> : null}{programId ? <Link href={`/spokedu-master/report?program=${programId}`} className="flex h-12 items-center justify-center rounded-[12px] text-[13px] font-bold" style={{ background: 'rgba(255,255,255,0.08)', color: 'var(--spm-t)' }}>설명 문구</Link> : null}<Link href="/spokedu-master/spomove" className="flex h-12 items-center justify-center rounded-[12px] text-[14px] font-bold" style={{ background: 'rgba(255,255,255,0.08)', color: 'var(--spm-t)' }}>목록으로</Link></div></div> : null}
