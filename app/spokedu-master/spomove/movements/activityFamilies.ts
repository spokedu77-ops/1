import type {
  ActivityFamilyDefinition,
  ActivityFamilyId,
  MovementPick,
  MovementProfileId,
} from './movementTypes';
import type { ActivityOperationProfileId } from '../operations/operationTypes';

function pick(baseMovement: MovementPick['baseMovement'], limbRule: MovementPick['limbRule']): MovementPick {
  return { baseMovement, limbRule };
}

function family(
  id: ActivityFamilyId,
  movementProfileId: MovementProfileId,
  operationProfileId: ActivityOperationProfileId,
  options?: {
    minMats?: number;
    recommendedMovement?: MovementPick;
    excludedMovements?: MovementPick[];
  },
): ActivityFamilyDefinition {
  return {
    id,
    movementProfileId,
    operationProfileId,
    matRequirement: { minMats: options?.minMats ?? 1 },
    recommendedMovement: options?.recommendedMovement,
    excludedMovements: options?.excludedMovements,
  };
}

/** 인지 고난도에서 기본 제외하는 교차·고강도 조합 (simpleColorResponse 허용 목록과 교차) */
const EXCLUDE_CROSS_AND_LUNGE: MovementPick[] = [
  pick('footTap', 'oppositeSide'),
  pick('handTouch', 'oppositeSide'),
  pick('lungeReach', 'free'),
];

/** 패밀리 레지스트리 — matRequirement·공식 추천·operationProfile의 단일 출처 */
export const ACTIVITY_FAMILIES: Record<string, ActivityFamilyDefinition> = {
  'reaction-space-direction': family('reaction-space-direction', 'simpleColorResponse', 'immediateResponse', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'reaction-quadrant': family('reaction-quadrant', 'simpleColorResponse', 'immediateResponse', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'reaction-full': family('reaction-full', 'themedFullResponse', 'immediateResponse', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'reaction-split': family('reaction-split', 'simpleColorResponse', 'immediateResponse', {
    recommendedMovement: pick('footTap', 'sameSide'),
  }),
  'reaction-triple': family('reaction-triple', 'simpleColorResponse', 'immediateResponse', {
    recommendedMovement: pick('footTap', 'free'),
    excludedMovements: [pick('lungeReach', 'free')],
  }),
  'reaction-triple-diff': family('reaction-triple-diff', 'simpleColorResponse', 'immediateResponse', {
    recommendedMovement: pick('handTouch', 'free'),
    excludedMovements: EXCLUDE_CROSS_AND_LUNGE,
  }),
  'reaction-variant-foot': family('reaction-variant-foot', 'variantFootFixed', 'legacyFixed', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'reaction-variant-body-cue': family('reaction-variant-body-cue', 'bodyCueBuiltIn', 'builtIn'),
  'visual-flow': family('visual-flow', 'visualSearch', 'legacyFixed', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'visual-flash': family('visual-flash', 'visualSearch', 'legacyFixed', {
    recommendedMovement: pick('handTouch', 'free'),
    excludedMovements: [pick('squatTouch', 'free')],
  }),
  'visual-pulse': family('visual-pulse', 'visualSearch', 'legacyFixed', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'visual-blackout': family('visual-blackout', 'visualSearch', 'legacyFixed', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'visual-rush': family('visual-rush', 'visualSearch', 'legacyFixed', {
    recommendedMovement: pick('handTouch', 'free'),
    excludedMovements: [pick('squatTouch', 'free'), pick('stepHold', 'free')],
  }),
  'visual-mole': family('visual-mole', 'visualSearch', 'legacyFixed', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'visual-wormhole': family('visual-wormhole', 'visualSearch', 'legacyFixed', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'visual-number-cart': family('visual-number-cart', 'gameSpecific', 'legacyFixed', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'visual-color-tracker': family('visual-color-tracker', 'gameSpecific', 'legacyFixed', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'visual-goalkeeper': family('visual-goalkeeper', 'visualSearch', 'legacyFixed', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'simon-pole': family('simon-pole', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'simon-mixed': family('simon-mixed', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'simon-balloon': family('simon-balloon', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'flanker-uniform': family('flanker-uniform', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'flanker-random': family('flanker-random', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'flanker-nested': family('flanker-nested', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'flanker-5circle': family('flanker-5circle', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'flanker-arrow': family('flanker-arrow', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'flanker-theme': family('flanker-theme', 'choiceReaction', 'choiceControl', {
    recommendedMovement: pick('footTap', 'free'),
  }),
  'stroop-arrow': family('stroop-arrow', 'complexReaction', 'choiceControl', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'stroop-word': family('stroop-word', 'complexReaction', 'choiceControl', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'stroop-missing': family('stroop-missing', 'complexReaction', 'choiceControl', {
    recommendedMovement: pick('handTouch', 'free'),
  }),
  'sequential-memory': family('sequential-memory', 'sequentialMemory', 'legacyFixed', {
    recommendedMovement: pick('stepHold', 'free'),
  }),
  dive: family('dive', 'diveBuiltIn', 'builtIn'),
};

/**
 * 레거시/코어 프리셋 enrichment 입력.
 * 최종 SSOT는 enrichment 후 `preset.activityFamilyId`.
 */
export const PRESET_FAMILY_MAP: Record<string, ActivityFamilyId> = {
  'reaction-cognition-space-direction-01': 'reaction-space-direction',
  'reaction-cognition-space-direction-color-01b': 'reaction-space-direction',
  'reaction-cognition-quad-color-02': 'reaction-quadrant',
  'reaction-cognition-quad-fruit-10': 'reaction-quadrant',
  'reaction-cognition-full-color-03': 'reaction-full',
  'reaction-cognition-full-animal-18': 'reaction-full',
  'reaction-cognition-full-nature-19': 'reaction-full',
  'reaction-cognition-split-color-04': 'reaction-split',
  'reaction-cognition-triple-color-25': 'reaction-triple',
  'reaction-cognition-triple-diff-color-31': 'reaction-triple-diff',
  'reaction-cognition-mq1-32': 'reaction-variant-foot',
  'reaction-cognition-mq2-33': 'reaction-variant-body-cue',
  'reaction-cognition-mq3-34': 'reaction-variant-body-cue',
  'visual-reaction-flow-2x-31': 'visual-flow',
  'visual-reaction-flash-33': 'visual-flash',
  'visual-reaction-blackout-37': 'visual-blackout',
  'visual-reaction-rush-39': 'visual-rush',
  'visual-reaction-mole-l1': 'visual-mole',
  'visual-reaction-number-cart-l2': 'visual-number-cart',
  'visual-reaction-color-tracker-l2': 'visual-color-tracker',
  'visual-reaction-goalkeeper-42': 'visual-goalkeeper',
  'visual-reaction-mole-normal-skeleton': 'visual-mole',
  'visual-reaction-goalkeeper-easy-skeleton': 'visual-goalkeeper',
  'visual-reaction-hand-foot-easy-skeleton': 'visual-flow',
  'visual-reaction-hand-foot-normal-skeleton': 'visual-flow',
  'visual-reaction-hand-foot-hard-skeleton': 'visual-flow',
  'visual-reaction-white-ball-hard-skeleton': 'visual-color-tracker',
  'simon-pole-shape-06': 'simon-pole',
  'simon-pole-arrows-41': 'simon-pole',
  'simon-mixed-gallery-exp': 'simon-mixed',
  'simon-balloon-flash-05': 'simon-balloon',
  'simon-arrow-hard-skeleton': 'simon-pole',
  'simon-shape-hard-skeleton': 'simon-pole',
  'simon-balloon-hard-skeleton': 'simon-balloon',
  'simon-random-hard-skeleton': 'simon-mixed',
  'simon-camouflage-center-skeleton': 'simon-mixed',
  'flanker-uniform-07': 'flanker-uniform',
  'flanker-uniform-number-exp': 'flanker-uniform',
  'flanker-random-43': 'flanker-random',
  'flanker-random-number-exp': 'flanker-random',
  'flanker-nested-circles-04': 'flanker-nested',
  'flanker-5circle-46': 'flanker-5circle',
  'flanker-5circle-number-exp': 'flanker-5circle',
  'flanker-arrow-05': 'flanker-arrow',
  'flanker-arrow-udlr-exp': 'flanker-arrow',
  'flanker-theme-06': 'flanker-theme',
  'flanker-theme-animal-skeleton': 'flanker-theme',
  'flanker-theme-color-skeleton': 'flanker-theme',
  'flanker-theme-food-skeleton': 'flanker-theme',
  'flanker-theme-nature-skeleton': 'flanker-theme',
  'flanker-theme-vehicle-skeleton': 'flanker-theme',
  'flanker-theme-mix-skeleton': 'flanker-theme',
  'flanker-extreme-arrow-hard-skeleton': 'flanker-arrow',
  'stroop-arrow-reverse-08': 'stroop-arrow',
  'stroop-arrow-bg-47': 'stroop-arrow',
  'stroop-word-reverse-48': 'stroop-word',
  'stroop-word-bg-49': 'stroop-word',
  'stroop-missing-color-50': 'stroop-missing',
  'sequential-memory-3color-09': 'sequential-memory',
  'sequential-memory-5color-51': 'sequential-memory',
  'sequential-memory-10color-52': 'sequential-memory',
  'sequential-memory-full-reveal-54': 'sequential-memory',
  'sequential-memory-color-number-exp': 'sequential-memory',
  'sequential-memory-custom-10color-exp': 'sequential-memory',
  'dive-standard': 'dive',
  'dive-random': 'dive',
  'dive-color-gate-61': 'dive',
};

const LEVEL_FAMILY: Record<number, ActivityFamilyId> = {
  2: 'reaction-quadrant',
  3: 'reaction-full',
  4: 'reaction-split',
  5: 'reaction-triple',
  6: 'reaction-triple-diff',
};

/** Expansion 테마 ID → family (생성 시점과 동일 규칙) */
export function deriveFamilyIdForPresetId(presetId: string): ActivityFamilyId | null {
  const mapped = PRESET_FAMILY_MAP[presetId];
  if (mapped) return mapped;

  const themeMatch = /^reaction-cognition-l([2-6])-[a-z]+-exp$/.exec(presetId);
  if (themeMatch) {
    const level = Number(themeMatch[1]);
    return LEVEL_FAMILY[level] ?? null;
  }

  return null;
}

export function getActivityFamily(id: ActivityFamilyId): ActivityFamilyDefinition | null {
  return ACTIVITY_FAMILIES[id] ?? null;
}

export function listActivityFamilies(): ActivityFamilyDefinition[] {
  return Object.values(ACTIVITY_FAMILIES);
}
