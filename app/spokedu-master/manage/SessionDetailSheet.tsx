...
export function SessionDetailSheet({
  ...
  return <>
    <BottomSheet open title={title} headerTitle={<h2 className="flex items-center gap-3 text-[18px] font-bold text-slate-950"><span className="h-2.5 w-2.5 rotate-45 border border-slate-700" aria-hidden />{title}</h2>} size="session" onClose={requestClose} footer={
      status === 'scheduled' ? <div className="grid grid-cols-[auto_minmax(0,1fr)] gap-2 border-t border-slate-200 bg-white px-1 pb-[max(10px,env(safe-area-inset-bottom))] pt-2">{activeSession && dirty ? <button type="button" disabled={saving} onClick={() => void persist('scheduled')} className={`${SPM_SECONDARY_BTN} px-4`}>변경 저장</button> : null}<button type="button" disabled={saving || !classId} onClick={() => void persist(activeSession ? 'completed' : 'scheduled')} className={SPM_PRIMARY_BTN_FULL}>{saving ? '저장 중…' : activeSession ? MASTER_ACTION_COPY.completeSession : MASTER_ACTION_COPY.createSession}</button></div> : status === 'completed' && dirty ? <div className="border-t border-slate-200 bg-white px-1 pb-[max(10px,env(safe-area-inset-bottom))] pt-2"><button type="button" disabled={saving} onClick={() => void persist('completed')} className={SPM_PRIMARY_BTN_FULL}>변경 저장</button></div> : undefined
    }>
      <div data-session-detail className="space-y-2 pb-1">
        <style>{`[data-session-detail] textarea { min-height: 64px !important; max-height: 64px; padding-block: 8px !important; }`}</style>
        <section aria-labelledby="session-information-heading">
          <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 id="session-information-heading" className="text-[18px] font-bold leading-6 text-slate-950">{selectedClass?.name ?? '수업 정보'}</h3><span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${status === 'completed' ? 'bg-emerald-50 text-emerald-700' : status === 'cancelled' ? 'bg-rose-50 text-rose-700' : 'bg-blue-50 text-blue-700'}`}>{statusLabel(status)}</span></div><p className="mt-0.5 text-xs text-slate-500">{formatSeoulSessionDay(startAt.slice(0, 10), { month: 'long', day: 'numeric', weekday: 'long' })} · {formatSeoulSessionTime(seoulDateTimeInputToIso(startAt))}–{formatSeoulSessionTime(seoulDateTimeInputToIso(endAt))}</p></div><details className="relative shrink-0"><summary className="grid h-9 w-9 cursor-pointer list-none place-items-center rounded-[10px] text-slate-400 hover:bg-slate-100" aria-label="수업 관리 메뉴"><MoreHorizontal size={18} /></summary><div className="absolute right-0 top-10 z-30 min-w-40 rounded-xl border border-slate-200 bg-white p-1 shadow-lg"><button type="button" onClick={() => setScheduleOpen((open) => !open)} className="min-h-11 w-full rounded-lg px-3 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50">{scheduleOpen ? '일정 접기' : '일정 변경'}</button>{activeSession && status === 'scheduled' ? <button type="button" disabled={saving} onClick={() => { if (window.confirm('이 수업을 취소할까요?')) void persist('cancelled'); }} className="min-h-11 w-full rounded-lg px-3 text-left text-xs font-semibold text-rose-600 hover:bg-rose-50">수업 취소</button> : null}{activeSession && status === 'completed' ? <button type="button" disabled={saving} onClick={() => { if (window.confirm('완료를 취소하고 예정 상태로 되돌릴까요?')) void persist('scheduled'); }} className="min-h-11 w-full rounded-lg px-3 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50">완료 취소</button> : null}{activeSession && status === 'cancelled' ? <><button type="button" disabled={saving} onClick={() => void persist('scheduled')} className="min-h-11 w-full rounded-lg px-3 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50">취소 해제</button><button type="button" disabled={saving} onClick={() => { if (!window.confirm('취소된 수업을 삭제할까요?')) return; setSaving(true); void data.deleteCancelledSession(activeSession.id).then(onClose).catch((caught) => setError(getMasterRequestErrorMessage(caught) || '수업을 삭제하지 못했습니다.')).finally(() => setSaving(false)); }} className="min-h-11 w-full rounded-lg px-3 text-left text-xs font-semibold text-rose-600 hover:bg-rose-50">수업 삭제</button></> : null}</div></details></div>
          ...
          {!activeSession && scheduleOpen ? <Link href="/spokedu-master/classes?create=1" className="mt-1 inline-flex min-h-11 items-center text-sm font-semibold text-slate-500 hover:text-slate-950">+ 새 수업반 만들기</Link> : null}
        </section>

        <section aria-labelledby="session-activities-heading"><div className="flex items-center justify-between gap-3"><h3 id="session-activities-heading" className="text-base font-bold text-slate-950">수업 활동</h3>{actions.addActivities ? <button type="button" onClick={() => setPickerOpen(true)} className="inline-flex min-h-9 items-center gap-1 rounded-[10px] border border-slate-200 px-3 text-sm font-semibold text-blue-600 hover:bg-blue-50"><Plus size={16} />활동 추가</button> : null}</div><div className="mt-2 space-y-1.5">{programs.map((program, index) => {
          ...
          const content = <span className="min-w-0 flex-1"><span className="line-clamp-2 text-[14px] font-semibold leading-[18px] text-slate-950">{displayTitle}</span><span className="block text-[11px] font-medium text-slate-500">{program.sourceType === 'spomove' ? 'SPOMOVE' : '놀이체육'}</span></span>;
          return <div key={program.id} className="flex min-h-[48px] items-center gap-1 rounded-[10px] border border-slate-200 bg-white px-1.5"><button type="button" disabled={!activeSession || !actions.toggleActivityCompletion} onClick={() => void toggleProgram(program)} className="grid h-10 w-10 shrink-0 place-items-center rounded-lg" aria-label={`${displayTitle} ${program.isCompleted ? '실제 진행' : '미진행'}`}><span className={`grid h-5 w-5 place-items-center rounded-[4px] border ${program.isCompleted ? 'border-blue-600 bg-blue-600 text-white' : 'border-slate-300 bg-white text-transparent'}`}><Check size={14} /></span></button>{detailHref ? <Link href={detailHref} target={spomoveHref ? '_blank' : undefined} rel={spomoveHref ? 'noreferrer' : undefined} className="flex min-w-0 flex-1">{content}</Link> : content}{actions.reorderActivities ? <details className="group relative shrink-0"><summary className="grid h-10 w-8 cursor-pointer list-none place-items-center text-slate-500 hover:text-slate-900" aria-label="활동 관리"><ChevronRight size={17} /></summary><div className="absolute right-0 top-10 z-20 flex min-w-36 flex-col rounded-xl border border-slate-200 bg-white p-1 shadow-lg"><button type="button" disabled={index === 0 || saving} onClick={() => void moveProgram(index, -1)} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-left text-xs font-semibold text-slate-600 hover:bg-slate-50"><ChevronUp size={15} />위로 이동</button><button type="button" disabled={index === programs.length - 1 || saving} onClick={() => void moveProgram(index, 1)} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-left text-xs font-semibold text-slate-600 hover:bg-slate-50"><ChevronDown size={15} />아래로 이동</button><button type="button" disabled={saving} onClick={() => void removeProgram(program)} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-left text-xs font-semibold text-rose-600 hover:bg-rose-50"><Trash2 size={14} />활동 삭제</button></div></details> : null}</div>;
        })}{!programs.length ? <p className="py-4 text-sm text-slate-500">아직 담은 활동이 없습니다.</p> : null}</div></section>

        {activeSession && actions.editAttendance ? <section aria-labelledby="session-attendance-heading"><div className="flex items-center justify-between"><h3 id="session-attendance-heading" className="text-sm font-bold text-slate-950">출석 <span className="ml-2 text-sm font-bold text-slate-900">{roster.filter((student) => attendance[student.id] === 'present').length} / {roster.length}</span></h3><button type="button" onClick={() => { setAttendance(Object.fromEntries(roster.map((student) => [student.id, 'present' as const]))); setDirty(true); }} className="h-8 rounded-[9px] border border-slate-200 px-3 text-[11px] font-semibold text-slate-600">전체 출석</button></div><div className="divide-y divide-slate-100">{roster.map((student) => <div key={student.id} className="flex min-h-[39px] items-center gap-2"><span className="min-w-0 flex-1 truncate text-xs font-medium text-slate-700">{student.name}</span>{(['present', 'absent'] as const).map((value) => <button key={value} type="button" onClick={() => { setAttendance((current) => current[student.id] === value ? Object.fromEntries(Object.entries(current).filter(([id]) => id !== student.id)) : { ...current, [student.id]: value }); setDirty(true); }} className={`h-7 min-w-[68px] rounded-full px-3 text-[11px] font-semibold ${attendance[student.id] === value ? value === 'present' ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white' : 'bg-slate-100 text-slate-500'}`}>{value === 'present' ? '출석' : '결석'}</button>)}</div>)}{!roster.length ? <p className="py-4 text-sm text-slate-500">등록된 학생이 없습니다.</p> : null}</div></section> : null}
