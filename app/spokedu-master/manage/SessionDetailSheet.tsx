...
import { Check, ChevronDown, ChevronRight, ChevronUp, MoreHorizontal, Plus, Save, Trash2 } from 'lucide-react';
...
export function SessionDetailSheet({
  ...
  const requestClose = () => {
    if (saving) return;
    if (dirty && !window.confirm('저장하지 않은 변경이 있습니다. 나가면 사라집니다.')) return;
    onClose();
  };
    ...
    <BottomSheet open title={title} size="session" onClose={requestClose} footer={
      ...
      <div className="space-y-7 pb-3">
        <section aria-labelledby="session-information-heading">
          <div className="flex items-start justify-between gap-3"><div><h3 id="session-information-heading" className="text-xl font-semibold text-slate-950">{selectedClass?.name ?? '수업 정보'}</h3><p className="mt-1 text-sm text-slate-500">{formatSeoulSessionDay(startAt.slice(0, 10), { month: 'long', day: 'numeric', weekday: 'long' })} · {formatSeoulSessionTime(seoulDateTimeInputToIso(startAt))}–{formatSeoulSessionTime(seoulDateTimeInputToIso(endAt))}</p><span className="mt-2 inline-block text-xs font-semibold text-slate-500">{statusLabel(status)}</span></div><button type="button" onClick={() => setScheduleOpen((open) => !open)} className="min-h-11 px-2 text-xs font-semibold text-slate-500" aria-expanded={scheduleOpen}>{scheduleOpen ? '접기' : '일정 변경'}</button></div>
        ...
        <section aria-labelledby="session-activities-heading"><div className="flex items-center justify-between gap-3"><h3 id="session-activities-heading" className="text-base font-semibold text-slate-950">수업 활동</h3>{activeSession && programs.length ? <span className="text-xs text-slate-400">진행 {programs.filter((item) => item.isCompleted).length}/{programs.length}</span> : null}</div><div className="mt-2 divide-y divide-slate-100">{programs.map((program, index) => {
          ...
          return <div key={program.id} className="flex min-h-16 items-center gap-2 py-2"><button type="button" disabled={!activeSession || !actions.toggleActivityCompletion} onClick={() => void toggleProgram(program)} className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl border ${program.isCompleted ? 'border-emerald-600 bg-emerald-600 text-white' : 'border-slate-300 bg-white text-slate-300'}`} aria-label={`${program.programTitle ?? '활동'} ${program.isCompleted ? '실제 진행' : '미진행'}`}><Check size={18} /></button>{detailHref ? <Link href={detailHref} target={spomoveHref ? '_blank' : undefined} rel={spomoveHref ? 'noreferrer' : undefined} className="flex min-w-0 flex-1 items-center gap-2"><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-800">{program.programTitle ?? '이름 없는 활동'}</span><span className="mt-0.5 block text-xs text-slate-400">{program.sourceType === 'spomove' ? 'SPOMOVE' : '놀이체육'}</span></span><ChevronRight size={17} className="shrink-0 text-slate-300" /></Link> : <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-slate-800">{program.programTitle ?? '이름 없는 활동'}</p><p className="mt-0.5 text-xs text-slate-400">{program.sourceType === 'spomove' ? 'SPOMOVE' : '놀이체육'}</p></div>}{actions.reorderActivities ? <details className="group relative shrink-0"><summary className="grid h-11 w-11 cursor-pointer list-none place-items-center rounded-xl text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="활동 관리"><MoreHorizontal size={18} /></summary><div className="absolute right-0 top-11 z-20 flex min-w-36 flex-col rounded-xl border border-slate-200 bg-white p-1 shadow-lg"><button type="button" disabled={index === 0 || saving} onClick={() => void moveProgram(index, -1)} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-left text-xs font-semibold text-slate-600 hover:bg-slate-50"><ChevronUp size={15} />위로 이동</button><button type="button" disabled={index === programs.length - 1 || saving} onClick={() => void moveProgram(index, 1)} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-left text-xs font-semibold text-slate-600 hover:bg-slate-50"><ChevronDown size={15} />아래로 이동</button><button type="button" disabled={saving} onClick={() => void removeProgram(program)} className="flex min-h-11 items-center gap-2 rounded-lg px-3 text-left text-xs font-semibold text-rose-600 hover:bg-rose-50"><Trash2 size={14} />활동 삭제</button></div></details> : null}</div>;
        ...
        {activeSession && actions.editAttendance ? <section aria-labelledby="session-attendance-heading"><div className="flex items-center justify-between"><h3 id="session-attendance-heading" className="text-base font-semibold text-slate-950">출석 <span className="ml-2 text-sm font-normal text-slate-500">{roster.filter((student) => attendance[student.id] === 'present').length} / {roster.length}</span></h3><button type="button" onClick={() => { setAttendance(Object.fromEntries(roster.map((student) => [student.id, 'present' as const]))); setDirty(true); }} className="min-h-11 px-2 text-xs font-semibold text-slate-600">전체 출석</button></div><div className="divide-y divide-slate-100">{roster.map((student) => <div key={student.id} className="flex min-h-14 items-center gap-3"><span className="min-w-0 flex-1 truncate text-sm font-medium text-slate-800">{student.name}</span>{(['present', 'absent'] as const).map((value) => <button key={value} type="button" onClick={() => { setAttendance((current) => current[student.id] === value ? Object.fromEntries(Object.entries(current).filter(([id]) => id !== student.id)) : { ...current, [student.id]: value }); setDirty(true); }} className={`min-h-11 rounded-xl px-3 text-xs font-semibold ${attendance[student.id] === value ? value === 'present' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-700' : 'bg-slate-50 text-slate-500'}`}>{value === 'present' ? '출석' : '결석'}</button>)}</div>)}{!roster.length ? <p className="py-4 text-sm text-slate-500">등록된 학생이 없습니다.</p> : null}</div></section> : null}

        {canUseRecords && status !== 'cancelled' ? <details><summary className="flex min-h-11 cursor-pointer items-center justify-between text-sm font-semibold text-slate-700">메모 <span className="text-xs font-normal text-slate-400">선택사항</span></summary><textarea value={memo} onChange={(event) => { setMemo(event.target.value); setDirty(true); }} placeholder="수업에 대한 메모를 남겨보세요." className="mt-2 min-h-20 w-full rounded-xl border border-slate-200 p-3 text-sm" /></details> : null}
