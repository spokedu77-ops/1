...
export function SessionActions({ status, isCreate, activeSession, dirty, saving, classId, persist }: { status: MasterSessionStatus; isCreate: boolean; activeSession: MasterSessionDto | null; dirty: boolean; saving: boolean; classId: string; persist: (status: MasterSessionStatus) => Promise<void> }) {
  ...
  if (status === 'completed' && dirty) return <div className="grid gap-2 border-t border-slate-200 bg-white px-1 pb-[max(10px,env(safe-area-inset-bottom))] pt-2">
    <button type="button" disabled={saving} onClick={() => void persist('completed')} className={SPM_PRIMARY_BTN_FULL}>변경 저장</button>
  </div>;
  return null;
}
