...
export function ScheduleTab({
  ...
  return <section className="mt-4 lg:flex lg:min-h-0 lg:flex-1 lg:flex-col" aria-labelledby="manage-calendar-heading">
    ...
    <div className="mt-4 flex shrink-0 flex-wrap items-center justify-between gap-x-4 gap-y-2 border-b border-slate-200 pb-2">
    ...
    <div data-manage-agenda data-agenda-count={daySessions.length} className={`min-h-0 divide-y divide-slate-100 ${detailOpen && daySessions.length > 1 ? 'lg:grid lg:grid-cols-2 lg:gap-x-3 lg:divide-y-0' : ''} ${daySessions.length > (detailOpen ? 4 : 2) ? 'lg:overflow-y-auto' : ''}`}>
      {daySessions.map((session) => {
        ...
        return <button key={session.id} type="button" onClick={() => onSessionSelect(session)} className={`grid w-full items-center rounded-[10px] text-left transition-colors hover:bg-white ${detailOpen ? 'min-h-[68px] grid-cols-[minmax(0,1fr)_auto] gap-x-2 gap-y-1 px-2 py-2.5' : 'min-h-[72px] grid-cols-[auto_minmax(0,1fr)_auto] gap-4 px-2 py-3'}`}>{detailOpen ? <><p className="truncate text-[15px] font-semibold leading-5 text-slate-950">{session.className}</p><ChevronRight size={18} className="row-span-2 self-center text-slate-400" /><div className="flex min-w-0 items-center gap-2"><span className="shrink-0 text-[13px] font-medium tabular-nums text-slate-600">{formatSeoulSessionTime(session.startAt)}–{formatSeoulSessionTime(session.endAt)}</span><span className={`inline-flex shrink-0 items-center gap-1.5 text-xs font-medium ${sessionStatusText(session.status)}`}><i className={`h-1.5 w-1.5 rounded-full ${sessionStatusDot(session.status)}`} />{status}</span></div></> : <><div><p className="text-[14px] font-medium tabular-nums text-slate-700">{formatSeoulSessionTime(session.startAt)}–{formatSeoulSessionTime(session.endAt)}</p><span className={`mt-1.5 inline-flex items-center gap-1.5 text-xs font-medium ${sessionStatusText(session.status)}`}><i className={`h-1.5 w-1.5 rounded-full ${sessionStatusDot(session.status)}`} />{status}</span></div><div className="min-w-0"><p className="truncate text-[16px] font-semibold text-slate-950">{session.className}</p><p className="mt-1 text-[13px] font-normal text-slate-500">{[programCount ? `놀이체육 ${programCount}` : '', spomoveCount ? `SPOMOVE ${spomoveCount}` : ''].filter(Boolean).join(' · ') || '활동 미정'}</p></div><ChevronRight size={18} className="shrink-0 text-slate-400" /></>}</button>;
      })}
