...
export function ScheduleTab({
  ...
  return <section className="mt-5" aria-labelledby="manage-calendar-heading">
    <style>{`[data-manage-agenda] > button { min-height: 60px !important; padding-block: 6px !important; }`}</style>
    ...
    <div className="mt-6 flex items-center justify-between gap-4"><h3 className="text-[21px] font-bold text-slate-950">{formatSeoulSessionDay(selectedDay, { month: 'long', day: 'numeric', weekday: 'long' })}</h3>{daySessions.length ? <span className="text-base font-semibold text-slate-500">수업 {daySessions.length}개</span> : null}</div>
    <div data-manage-agenda className="mt-3 space-y-2">
