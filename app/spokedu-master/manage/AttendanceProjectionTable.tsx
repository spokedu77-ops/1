...
export function AttendanceProjectionTable({
  ...
  if (!sessions.length) {
    return <div className="py-10 text-center"><p className="text-sm font-semibold text-slate-700">아직 {emptyMonthLabel ?? '이 달'}에 완료된 수업이 없습니다.</p><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">수업을 완료하면 날짜별 출석 기록을 이곳에 자동으로 정리합니다.</p>{emptyAction}</div>;
  }
            ...
            {sessions.map((session) => (
              <th key={session.id} className="min-w-20 border-b border-slate-200 bg-slate-50 px-3 py-2 text-center">
                <Link href={`/spokedu-master/activity?session=${encodeURIComponent(session.id)}`} className="block min-h-11 font-semibold text-slate-700" title={`${formatSeoulSessionTime(session.startAt)} 수업 열기`}>
                  ...
                  <small className="block text-[10px] font-medium text-slate-400">{formatSeoulSessionTime(session.startAt)}</small>
                </Link>
              </th>
            ))}
          </tr>
            ...
            <tr key={row.studentId}>
              <th className="sticky left-0 z-10 border-b border-r border-slate-100 bg-white px-3 py-3 text-left">
                {row.current ? (
                  ...
                  <><span className="block max-w-28 truncate font-semibold text-slate-800" title={row.studentName}>{row.studentName}</span><small className="text-[10px] font-medium text-slate-400">과거 참여</small></>
                )}
              </th>
              {sessions.map((session) => {
