...
function Stat({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[12px] p-3 text-center" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}><p className="text-[20px] font-black" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)' }}>{value}</p><p className="mt-1 text-[10px] font-semibold" style={{ color: 'var(--spm-t3)' }}>{label}</p></div>;
}
...
function PlanCard({ id, selected, onSelect }: { id: PlanType; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <button type="button" onClick={onSelect} className="w-full rounded-[16px] p-4 text-left transition" style={{ background: selected ? 'rgba(99,102,241,0.16)' : 'var(--spm-s2)', border: selected ? '1px solid rgba(99,102,241,0.55)' : '1px solid var(--spm-br2)' }}>
      <div className="flex items-start justify-between gap-3"><div><span className="text-[10px] font-black uppercase tracking-[0.12em]" style={{ color: selected ? '#a5b4fc' : 'var(--spm-t3)' }}>{plan.badge}</span><h3 className="mt-1 text-[18px] font-black" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)', letterSpacing: 0 }}>{plan.title}</h3></div><span className="text-[14px] font-black" style={{ color: 'var(--spm-t)' }}>{plan.price}</span></div>
      <div className="mt-4 grid grid-cols-3 gap-2 text-center"><span className="rounded-[10px] p-2 text-[11px] font-bold" style={{ background: 'var(--spm-s3)', color: 'var(--spm-t2)' }}>카카오 {plan.kakao}</span><span className="rounded-[10px] p-2 text-[11px] font-bold" style={{ background: 'var(--spm-s3)', color: 'var(--spm-t2)' }}>AI {plan.ai}</span><span className="rounded-[10px] p-2 text-[11px] font-bold" style={{ background: 'var(--spm-s3)', color: 'var(--spm-t2)' }}>PDF {plan.pdf}</span></div>
...
function PaymentFlow({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <BottomSheet open={open} title="구독 플랜 선택" onClose={close}>
      {step === 0 ? <div className="space-y-3">{(['free', 'pro', 'team'] as PlanType[]).map((id) => <PlanCard key={id} id={id} selected={selectedPlan === id} onSelect={() => setSelectedPlan(id)} />)}<button type="button" onClick={() => (selectedPlan === 'free' ? (setProfile({ plan: 'free' }), close()) : setStep(1))} className="h-12 w-full rounded-[12px] text-[14px] font-black text-white" style={{ background: 'var(--spm-acc)' }}>{selectedPlan === 'free' ? '무료 플랜으로 변경' : '결제 정보 입력'}</button></div> : null}
      {step === 1 ? <div className="space-y-5"><div className="grid grid-cols-3 gap-2">{[['card', '카드'], ['kakao', '카카오페이'], ['naver', '네이버페이']].map(([id, label]) => <button key={id} type="button" onClick={() => setMethod(id as 'card' | 'kakao' | 'naver')} className="h-10 rounded-[12px] text-[12px] font-black" style={{ background: method === id ? 'var(--spm-acc)' : 'var(--spm-s2)', color: method === id ? '#fff' : 'var(--spm-t2)', border: '1px solid var(--spm-br2)' }}>{label}</button>)}</div><input value={cardNumber} onChange={(event) => setCardNumber(event.target.value.replace(/\D/g, '').slice(0, 16).replace(/(\d{4})(?=\d)/g, '$1 '))} placeholder="1234 5678 9012 3456" className="h-12 w-full rounded-[12px] border px-3 text-[15px] font-bold outline-none" style={{ background: 'var(--spm-s2)', borderColor: 'var(--spm-br2)', color: 'var(--spm-t)' }} /><button type="button" onClick={() => { setProfile({ plan: selectedPlan, role: selectedPlan === 'team' ? 'director' : profile?.role ?? 'teacher' }); setStep(2); }} className="h-12 w-full rounded-[12px] text-[14px] font-black text-white" style={{ background: 'var(--spm-acc)' }}>{PLAN_DETAILS[selectedPlan].title} 시작</button></div> : null}
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      ...
      <main className="grid gap-7 px-[22px] sm:px-8 lg:grid-cols-[minmax(0,1fr)_420px] lg:px-10">
        ...
        <div className="space-y-7"><MenuGroup title="수업 실행"><MenuRow icon={MonitorPlay} label="SPOMOVE 전체화면 실행" href="/spokedu-master/spomove/session?mode=projector" /><MenuRow icon={ShoppingBag} label="교구 쇼핑 / 장바구니" href="/spokedu-master/shop" /><MenuRow icon={CreditCard} label="구독/결제 관리" onClick={() => setPaymentOpen(true)} /></MenuGroup><MenuGroup title="알림과 환경"><MenuRow icon={Bell} label="카카오/앱 알림 설정" /><MenuRow icon={Moon} label="수업 중 방해금지" /><MenuRow icon={Volume2} label="SPOMOVE 효과음 설정" /></MenuGroup><MenuGroup title="지원"><MenuRow icon={HelpCircle} label="도움말" /><MenuRow icon={Mail} label="문의하기" href="mailto:support@spokedu.com" /></MenuGroup><MenuGroup title="계정"><MenuRow icon={LogOut} label="로그아웃" /><MenuRow icon={Trash2} label="계정 삭제" danger /></MenuGroup></div>
      </main>
      ...
      <div className="px-[22px] pt-6 sm:px-8 lg:px-10"><p className="flex items-center gap-2 text-[11px]" style={{ color: 'var(--spm-t3)' }}><ShieldCheck size={13} />현재 화면은 MASTER 프로토타입용 결제 흐름입니다. 실제 결제, 카카오 발송, PDF 생성은 백엔드 연동 단계에서 활성화됩니다.</p></div>
    </div>
  );
}
