...
function SettingRow({ icon: Icon, label, enabled, onToggle }: { icon: LucideIcon; label: string; enabled: boolean; onToggle: () => void }) {
  return (
    <button type="button" onClick={onToggle} className="flex w-full items-center gap-3 rounded-[12px] p-3" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br)' }}>
      <span className="grid h-9 w-9 shrink-0 place-items-center rounded-[10px]" style={{ background: 'var(--spm-s3)' }}>
        <Icon size={17} color={enabled ? 'var(--spm-acc)' : 'var(--spm-t3)'} />
      </span>
    ...
    <BottomSheet open={open} title="구독 플랜 선택" onClose={close}>
      ...
      {step === 1 ? (
        <div className="space-y-5">
          ...
          <input value={cardNumber} onChange={(event) => setCardNumber(event.target.value.replace(/\D/g, '').slice(0, 16).replace(/(\d{4})(?=\d)/g, '$1 '))} placeholder="카드번호 입력" inputMode="numeric" className="h-12 w-full rounded-[12px] border px-3 text-[15px] font-bold outline-none" style={{ background: 'var(--spm-s2)', borderColor: method === 'card' && cardDigits.length > 0 && cardDigits.length < 16 ? 'var(--spm-amb)' : 'var(--spm-br2)', color: 'var(--spm-t)' }} />
      ...
      {step === 2 ? (
        <div className="py-5 text-center">
          <div className="mx-auto grid h-16 w-16 place-items-center rounded-full" style={{ background: 'rgba(16,185,129,0.14)' }}><CreditCard size={28} color="var(--spm-grn)" /></div>
  ...
  return (
    <BottomSheet open={open} title="구독 해지 확인" onClose={onClose}>
      <div className="space-y-4">
        ...
        {pauseRequested ? <p className="rounded-[12px] p-3 text-[12px] font-bold" style={{ background: 'rgba(245,158,11,0.12)', color: 'var(--spm-amb)' }}>1개월 일시정지 요청이 준비되었습니다. 데이터 보존 상태로 전환됩니다.</p> : null}
        <div className="grid gap-2">
          <button type="button" onClick={() => setPauseRequested(true)} className="flex items-center gap-3 rounded-[12px] p-3 text-left" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
            <PauseCircle size={18} color="var(--spm-amb)" />
            <span><span className="block text-[13px] font-black" style={{ color: 'var(--spm-t)' }}>1개월 일시정지</span><span className="text-[11px]" style={{ color: 'var(--spm-t3)' }}>비수기에는 데이터를 보존만 합니다.</span></span>
          </button>
          <button type="button" onClick={() => { setProfile({ plan: 'pro', role: 'teacher' }); onClose(); }} className="flex items-center gap-3 rounded-[12px] p-3 text-left" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
            ...
            <span><span className="block text-[13px] font-black" style={{ color: 'var(--spm-t)' }}>개인 플랜으로 다운그레이드</span><span className="text-[11px]" style={{ color: 'var(--spm-t3)' }}>센터 기능만 줄이고 강사 기록 루프는 유지합니다.</span></span>
          </button>
        </div>
...
export default function SpokeduMasterProfilePage() {
  ...
  const daysLeft = useMemo(() => profile?.trialEndsAt ? Math.max(0, Math.ceil((new Date(profile.trialEndsAt).getTime() - Date.now()) / (24 * 60 * 60 * 1000))) : 0, [profile?.trialEndsAt]);
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      ...
      <main className="grid gap-7 px-[22px] sm:px-8 lg:grid-cols-[minmax(0,1fr)_420px] lg:px-10">
        <div className="space-y-7">
          <section className="overflow-hidden rounded-[18px] p-5" style={{ background: currentPlan === 'free' ? 'linear-gradient(135deg, rgba(245,158,11,0.14), var(--spm-s2))' : 'linear-gradient(135deg, rgba(99,102,241,0.24), var(--spm-s2))', border: currentPlan === 'free' ? '1px solid rgba(245,158,11,0.24)' : '1px solid rgba(99,102,241,0.34)' }}>
            ...
            <div className="mt-5 grid gap-2 sm:grid-cols-2">
              ...
              {currentPlan !== 'free' ? <button type="button" onClick={() => setCancelOpen(true)} className="h-12 rounded-[12px] text-[14px] font-black" style={{ background: 'var(--spm-s3)', color: 'var(--spm-t2)', border: '1px solid var(--spm-br2)' }}>해지/일시정지</button> : null}
            </div>
          </section>
          ...
          <OperationsPanel />
          <section className="rounded-[18px] p-5" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
            <div className="flex items-center gap-2"><WifiOff size={18} color="var(--spm-amb)" /><h2 className="text-[17px] font-black" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)', letterSpacing: 0 }}>오프라인/한도 정책</h2></div>
        ...
        <div className="space-y-7">
          ...
          <MenuGroup title="알림과 환경">
            <SettingRow icon={Bell} label="카카오 요약 알림" enabled={notificationOn} onToggle={() => setNotificationOn((value) => !value)} />
            <SettingRow icon={Moon} label="수업 중 방해금지" enabled={dndOn} onToggle={() => setDndOn((value) => !value)} />
            <SettingRow icon={Volume2} label="SPOMOVE 효과음" enabled={soundOn} onToggle={() => setSoundOn((value) => !value)} />
          </MenuGroup>
          <MenuGroup title="지원">
            <MenuRow icon={HelpCircle} label="고객지원" onClick={() => setAccountNotice('고객지원 요청이 준비되었습니다. 문의하기로 상세 내용을 보낼 수 있습니다.')} />
            <MenuRow icon={Mail} label="문의하기" href="mailto:support@spokedu.com" />
          </MenuGroup>
          <MenuGroup title="계정">
            <MenuRow icon={LogOut} label="로그아웃" onClick={() => setAccountNotice('현재 기기에서 MASTER 세션 종료 요청이 준비되었습니다.')} />
            <MenuRow icon={Trash2} label="계정 삭제" danger onClick={() => setAccountNotice('계정 삭제는 학생 데이터 보존 정책 확인 후 진행해야 합니다.')} />
          </MenuGroup>
      ...
      <div className="px-[22px] pt-6 sm:px-8 lg:px-10">
        <p className="flex items-center gap-2 text-[11px]" style={{ color: 'var(--spm-t3)' }}><ShieldCheck size={13} />결제 정보와 학생 성장 데이터는 플랜 권한에 맞춰 안전하게 관리됩니다.</p>
      </div>
    </div>
  );
}
