...
import { ArrowRight, Check, School, Sparkles, UserRound, UsersRound } from 'lucide-react';
...
function StepDot({ active, done }: { active: boolean; done: boolean }) {
  return (
    <span
      ...
      style={{ background: active ? 'var(--spm-acc)' : done ? 'var(--spm-grn)' : 'var(--spm-s4)' }}
    />
  );
}

function ChoiceCard({ title, desc, active, icon: Icon, onClick }: { title: string; desc: string; active: boolean; icon: typeof UserRound; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-start gap-3 rounded-[16px] p-4 text-left"
      style={{ background: active ? 'rgba(99,102,241,0.15)' : 'var(--spm-s2)', border: active ? '1px solid rgba(99,102,241,0.55)' : '1px solid var(--spm-br2)' }}
    >
...
function ToggleChip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button
      ...
      style={{ background: active ? 'var(--spm-acc)' : 'var(--spm-s2)', color: active ? '#fff' : 'var(--spm-t2)', border: active ? '1px solid transparent' : '1px solid var(--spm-br2)' }}
    >
      {label}
    </button>
  );
...
export default function OnboardingPage() {
  ...
  const centerValid = centerMode === 'personal' || centerCode.trim().length >= 10;
  const profileValid = name.trim().length > 0 && name.trim().length <= 20;
  const canNext = useMemo(() => {
    if (step === 0) return !!role;
    if (step === 1) return centerValid;
    if (step === 2) return profileValid;
    return true;
  }, [centerValid, profileValid, role, step]);
  ...
  const finish = () => {
    const mockCenterName = centerMode === 'center' ? '스포무브 데모 센터' : null;
    setProfile({
      name: name.trim() || '선생님',
      school: school.trim(),
      role,
      centerId: centerMode === 'center' ? centerCode.trim().toUpperCase() : null,
    ...
    <div className="h-full overflow-y-auto pb-8" style={{ background: 'var(--spm-bg)' }}>
      <main className="mx-auto flex min-h-full w-full max-w-[880px] flex-col justify-center px-[22px] py-8 sm:px-8">
        <div className="mb-8">
          ...
          <h1 className="mt-3 text-[34px] font-black leading-[1.12] md:text-[48px]" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)', letterSpacing: 0, wordBreak: 'keep-all' }}>
            수업 준비 30초, 기록 3분으로 시작합니다
          </h1>
          <p className="mt-3 max-w-[640px] text-[14px] font-medium leading-7" style={{ color: 'var(--spm-t2)' }}>
            강사·원장 역할을 먼저 정하고, 개인 또는 센터 플랜으로 MASTER를 시작합니다.
          </p>
        </div>

        <div className="mb-6 flex gap-2">
          {[0, 1, 2, 3].map((item) => <StepDot key={item} active={step === item} done={step > item} />)}
        </div>
        ...
        <section className="rounded-[20px] p-5" style={{ background: 'var(--spm-s1)', border: '1px solid var(--spm-br2)' }}>
          {step === 0 ? (
            <div className="space-y-3">
              ...
              <ChoiceCard title="원장으로 시작" desc="센터 플랜, 강사 기록율, 이탈 학생, 구독 현황을 관리합니다." active={role === 'director'} icon={UsersRound} onClick={() => setRole('director')} />
            </div>
          ) : null}
          ...
          {step === 1 ? (
            <div className="space-y-4">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <ChoiceCard title="개인으로 시작" desc="14일 무료 체험으로 혼자 먼저 써봅니다." active={centerMode === 'personal'} icon={UserRound} onClick={() => setCenterMode('personal')} />
              ...
              {centerMode === 'center' ? (
                <label className="block">
                  ...
                  <input value={centerCode} onChange={(event) => setCenterCode(event.target.value)} placeholder="예: SPOMOVE2026" className="h-12 w-full rounded-[12px] border px-3 text-[14px] font-bold uppercase outline-none" style={{ background: 'var(--spm-s2)', borderColor: centerValid ? 'var(--spm-br2)' : 'var(--spm-red)', color: 'var(--spm-t)' }} />
                  {!centerValid ? <p className="mt-2 text-[11px] font-bold" style={{ color: 'var(--spm-red)' }}>센터 코드는 10자 이상이어야 합니다.</p> : null}
                </label>
              ) : null}
            </div>
          ) : null}
          ...
          {step === 3 ? (
            <div className="space-y-4">
              ...
              <p className="text-[13px] font-medium leading-6" style={{ color: 'var(--spm-t2)' }}>
                이제 라이브러리에서 수업을 고르고, SPOMOVE를 실행하고, 수업 기록을 학생 성장 이력으로 쌓을 수 있습니다.
              </p>
              <div className="grid gap-2 sm:grid-cols-3">
                {['프로그램 추천', '수업 기록', '카카오 공유'].map((item) => (
                  <div key={item} className="rounded-[12px] p-3 text-center text-[12px] font-black" style={{ background: 'var(--spm-s2)', color: 'var(--spm-t)' }}>{item}</div>
                ))}
              </div>
            </div>
          ) : null}
          ...
          <div className="mt-6 grid grid-cols-[auto_1fr] gap-2">
            <button type="button" onClick={() => setStep((value) => Math.max(0, value - 1))} disabled={step === 0} className="h-12 rounded-[12px] px-5 text-[13px] font-black disabled:opacity-40" style={{ background: 'var(--spm-s2)', color: 'var(--spm-t)' }}>
              이전
            </button>
            <button type="button" onClick={step === 3 ? finish : () => setStep((value) => Math.min(3, value + 1))} disabled={!canNext} className="flex h-12 items-center justify-center gap-2 rounded-[12px] text-[14px] font-black text-white disabled:opacity-50" style={{ background: 'var(--spm-acc)' }}>
              {step === 3 ? 'MASTER 시작' : '다음'}
              <ArrowRight size={16} />
            </button>
          </div>
