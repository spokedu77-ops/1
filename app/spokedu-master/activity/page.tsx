...
import {
  ArrowLeft, CalendarDays, Check, CheckCircle2, ChevronDown, ChevronRight, ChevronUp, Clock3,
  ...
  return (
    <main className="h-full overflow-y-auto bg-[var(--spm-bg)] pb-28 lg:pb-8">
      <MasterPageShell variant="operational">
        <MasterPageHeader
          title="수업 관리"
          description={`${formatSeoulSessionDay(selectedDay, { year: 'numeric', month: 'long', day: 'numeric', weekday: 'short' })} 일정`}
        ...
        {!data.classes.length && data.status === 'ready' ? <MasterState kind="attention" title="수업반을 만들면 캘린더에 일정을 추가할 수 있습니다." className="mt-6" /> : null}

        <MasterSection title="수업 캘린더" className="mt-8" titleId="manage-calendar-heading">
          ...
          <div className="mt-5 flex items-center justify-between gap-3">
            <h3 className="text-base font-semibold text-slate-950">{formatSeoulSessionDay(selectedDay, { month: 'long', day: 'numeric', weekday: 'long' })}</h3>
            <span className="text-xs font-medium text-slate-500">수업 {daySessions.length}개</span>
          </div>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {daySessions.map((session) => {
              ...
              return <button key={session.id} type="button" onClick={() => setEditing(session)} className={`rounded-2xl border bg-white p-4 text-left transition hover:-translate-y-px hover:shadow-sm ${debt ? 'border-amber-200' : 'border-slate-200'}`}>
                <div className="flex items-center justify-between gap-2"><span className="flex items-center gap-1.5 text-sm font-semibold text-slate-800"><Clock3 size={15} />{formatSeoulSessionTime(session.startAt)}–{formatSeoulSessionTime(session.endAt)}</span><span className={`rounded-full px-2 py-1 text-[10px] font-semibold ring-1 ${statusTone(session.status)}`}>{statusLabel(session.status)}</span></div>
                <h4 className="mt-2 text-base font-semibold text-slate-950">{session.className}</h4>
                <p className={`mt-1 text-xs font-medium ${debt ? 'text-amber-700' : 'text-slate-500'}`}>{workState.operationalLabel}</p>
              </button>;
            })}
            {!daySessions.length ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-7 text-center sm:col-span-2"><UsersRound className="mx-auto text-slate-300" /><p className="mt-3 text-sm font-medium text-slate-500">이 날짜에 예정된 수업이 없습니다.</p><button type="button" onClick={() => { setCreateClassId(null); setEditing(null); }} disabled={!data.classes.length} className={`mt-4 ${SPM_PRIMARY_BTN_TALL}`}><Plus size={16} />이 날짜에 수업 추가</button></div> : null}
          </div>
        </MasterSection>
