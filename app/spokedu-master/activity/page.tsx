      ...
      <main className="grid gap-5 px-[22px] sm:px-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-10">
        <section className="grid gap-3 md:grid-cols-3 lg:col-span-2">
          {[
            ...
            { href: '/spokedu-master/report', title: '안내문', desc: '수업 기록을 바탕으로 전달용 안내문을 작성합니다.', Icon: FileText },
        ...
        <section className="rounded-[18px] p-5" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          ...
          {records.length ? (
            <div className="mt-4 grid gap-2">
              {records.slice(0, 3).map((record) => (
                <Link key={record.id} href={`/spokedu-master/class-record?record=${record.id}&program=${record.programId}`} className="rounded-[13px] p-3" style={{ background: 'var(--spm-s3)' }}>
                  <p className="text-[13px] font-black" style={{ color: 'var(--spm-t)' }}>{record.programTitle}</p>
            ...
            <div className="mt-4 rounded-[13px] p-4" style={{ background: 'var(--spm-s3)' }}>
              <p className="text-[14px] font-black" style={{ color: 'var(--spm-t)' }}>아직 작성한 수업 기록이 없습니다.</p>
              <p className="mt-2 text-[12px] font-semibold leading-5" style={{ color: 'var(--spm-t2)' }}>수업을 마친 뒤 기록을 남겨 보세요.</p>
        ...
        <section className="rounded-[18px] p-5" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          ...
          {explanations.length ? (
            <div className="mt-4 grid gap-2">
              {explanations.slice(0, 3).map((item) => (
                <Link key={item.id} href={`/spokedu-master/report?saved=${item.id}&program=${item.programId}`} className="rounded-[13px] p-3" style={{ background: 'var(--spm-s3)' }}>
                  <p className="text-[13px] font-black" style={{ color: 'var(--spm-t)' }}>{item.programTitle}</p>
            ...
            <div className="mt-4 rounded-[13px] p-4" style={{ background: 'var(--spm-s3)' }}>
              <p className="text-[14px] font-black" style={{ color: 'var(--spm-t)' }}>아직 저장한 안내문이 없습니다.</p>
              <p className="mt-2 text-[12px] font-semibold leading-5" style={{ color: 'var(--spm-t2)' }}>수업 기록을 바탕으로 전달용 안내문을 작성할 수 있습니다.</p>
        ...
        <section className="rounded-[18px] p-5 lg:row-start-2" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          ...
          {students.length ? (
            ...
            <div className="mt-4 rounded-[13px] p-4" style={{ background: 'var(--spm-s3)' }}>
              <p className="text-[14px] font-black" style={{ color: 'var(--spm-t)' }}>아직 등록된 학생이 없습니다.</p>
        ...
        <section className="rounded-[18px] p-5 lg:col-span-2" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[12px] font-semibold leading-5" style={{ color: 'var(--spm-t2)' }}>수업 자료를 선택하고 실행한 뒤 기록과 안내문까지 이어가세요.</p>
