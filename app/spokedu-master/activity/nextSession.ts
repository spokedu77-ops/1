...
export function buildNextSessionDraft(sourceSession: MasterSessionDto, now = new Date()): NextSessionDraft {
  const startInput = toSeoulDateTimeInput(sourceSession.startAt);
  const endInput = toSeoulDateTimeInput(sourceSession.endAt);
  let day = addSeoulSessionDays(getSeoulSessionDay(sourceSession.startAt), 7);
  while (new Date(`${day}T${startInput.slice(11, 16)}:00+09:00`).getTime() <= now.getTime()) day = addSeoulSessionDays(day, 7);
  return {
    day,
