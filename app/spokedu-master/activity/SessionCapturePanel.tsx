...
export const SessionCapturePanel = forwardRef<
...
>(function SessionCapturePanel(
  ...
  useEffect(() => {
    ...
    void fetchSessionCaptures(
      ...
      if (
        !shouldApplyServerSessionCapture({
      ...
      setObservations(
        Object.fromEntries(
          current?.students
            .map((student) => [student.studentId, student.memo ?? ''])
            .filter(([id]) => id) ?? [],
        ),
      );
    });
    ...
    try {
      const saved = await saveSessionCapture({
        ...
        observations: Object.entries(observations)
          ...
          .map(([studentId, memo]) => ({ studentId, memo })),
      });
      setCaptures((items) => [
  ...
  function toggleEditor() {
    if (
      open &&
      dirty &&
      !window.confirm('저장하지 않은 수업 기록이 있습니다. 닫을까요?')
    )
      return;
    setOpen((value) => !value);
  }
  ...
  if (!canUseRecords) {
    ...
    return (
      <section
        data-session-capture-gate
        className={`${orderClass} rounded-xl border border-slate-200 bg-slate-50 p-3`}
      >
        <p className="text-sm font-semibold text-slate-800">
          오늘 관찰을 남기면 다음 준비에 이어집니다
        </p>
        <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">
          출석은 Lite에서도 저장됩니다. 학생별 관찰·다음 수업 메모는
          프리미엄에서 이어집니다.
        </p>
        <Link
  ...
  if (captureMode === 'emphasized') {
    return (
      <section
        data-session-capture
        data-capture-mode="emphasized"
        className={`${orderClass} divide-y divide-slate-200`}
      >
        <div className="pb-4">
          ...
          <label className="mt-3 block text-sm font-semibold text-slate-800">
            ...
            <textarea
              ...
              className={`mt-2 min-h-20 ${SPM_JOURNEY_FIELD}`}
              placeholder="오늘 수업에서 기억할 만한 일이 있었나요?"
              maxLength={2000}
            />
          </label>
        </div>
          ...
          <div className="py-4">
            <label className="block text-sm font-semibold text-slate-800">
              ...
              <textarea
                ...
                className={`mt-2 min-h-20 ${SPM_JOURNEY_FIELD}`}
                placeholder="다음 시간에는 공의 거리를 조금 늘려보기"
                maxLength={500}
              />
            </label>
          </div>
        ...
        {canUseRecords ? (
          ...
          >
            <summary className="flex min-h-12 cursor-pointer list-none items-center justify-between text-sm font-medium text-slate-700">
              <span>학생별 기록</span>
              <span className={SPM_JOURNEY_META}>
                {
                  Object.values(observations).filter((value) => value.trim())
                    .length
                }
                /{roster.length}명
              </span>
              ...
              {roster.map((student) => (
                ...
                >
                  ...
                  <textarea
                    ...
                    className={`mt-1 min-h-16 ${SPM_JOURNEY_FIELD}`}
                    placeholder="짧은 관찰 기록"
                    maxLength={1000}
                  />
                </label>
              ))}
        ...
        {loadError ? (
          <p role="status" className="py-3 text-xs font-medium text-amber-700">
            기존 기록을 불러오지 못했습니다. 메모 없이 마무리할 수 있습니다.
          </p>
        ) : null}
        {saveError ? (
          <p role="alert" className="py-3 text-xs font-bold text-rose-700">
            기록을 저장하지 못했습니다. 입력 내용은 유지됩니다.
          </p>
        ) : null}
        {saving ? (
  ...
  return (
    <section
      ...
      className={`${orderClass} rounded-xl border p-3 ${tone}`}
    >
      {loadError ? (
        <p role="status" className="mb-2 text-xs font-bold text-amber-700">
          지난 수업 기록을 불러오지 못했습니다. 수업 진행은 계속할 수 있습니다.
        </p>
      ) : null}
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-slate-500">
            {recordedCount}명 기록 · 다음 수업 메모{' '}
            {capture?.applicationIdea ? '있음' : '없음'}
          </p>
        </div>
        <button
          type="button"
      ...
      {open ? (
        <div className="mt-3 space-y-2 border-t border-slate-100 pt-3">
            ...
            >
              ...
              >
                <span>
                  {student.name}
                  {student.guidanceNote ? (
                    <small className="ml-2 text-amber-700">
                      지도 참고사항 있음
                    </small>
                  ) : null}
                </span>
                <span>
                  {observations[student.id]?.trim() ? '기록됨' : '메모 추가'}
                </span>
              </button>
          ...
          {saveError ? (
            <p
              role="alert"
              className="rounded-lg bg-rose-50 p-2 text-xs font-bold text-rose-700"
            >
              기록을 저장하지 못했습니다. 작성한 내용은 그대로 유지됩니다.
            </p>
          ) : null}
          <button
            type="button"
