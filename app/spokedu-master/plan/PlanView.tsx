  ...
  return (
    <section className="mx-[22px] mb-7 rounded-[16px] p-5 sm:mx-8 lg:mx-10" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.12em]" style={{ color: 'var(--spm-t3)' }}>semester progress</p>
          <h2 className="mt-1 text-[22px] font-black" style={{ fontFamily: 'var(--spm-font-display)', color: 'var(--spm-t)' }}>5월 2주차</h2>
        </div>
...
function AddLessonSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const save = () => {
    addLesson({ id: Date.now(), title: program.title, classId, date: new Date().toISOString(), period, duration, done: false, color: program.colors[1], memo: program.category });
    onClose();
  };
    ...
    <BottomSheet open={open} title="수업 추가" onClose={onClose}>
      <div className="space-y-5">
        <label className="block">
          <span className="mb-2 block text-[12px] font-bold" style={{ color: 'var(--spm-t3)' }}>프로그램</span>
          <select value={programId} onChange={(event) => setProgramId(event.target.value)} className="h-11 w-full rounded-[12px] border px-3 text-[14px] font-bold outline-none" style={{ background: 'var(--spm-s2)', borderColor: 'var(--spm-br2)', color: 'var(--spm-t)', colorScheme: 'dark' }}>
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      ...
      <section className="mx-[22px] mb-5 grid grid-cols-2 gap-2 rounded-[14px] p-1 sm:mx-8 lg:mx-10" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
        ...
        <button type="button" onClick={() => setMode('list')} className="flex h-10 items-center justify-center gap-2 rounded-[11px] text-[12px] font-black" style={{ background: mode === 'list' ? 'var(--spm-acc)' : 'transparent', color: mode === 'list' ? '#fff' : 'var(--spm-t3)' }}>
          <ListChecks size={15} />
          목록
        </button>
      </section>
        ...
        <section className="px-[22px] sm:px-8 lg:px-10">
          <div className="grid grid-cols-7 gap-1 rounded-[16px] p-3" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
            {weekDays.map((day) => {
              ...
              return (
                <button key={day.toISOString()} type="button" onClick={() => setSelectedDate(day)} className="relative flex h-[58px] flex-col items-center justify-center rounded-[12px] text-[11px] font-bold" style={{ background: selected || today ? 'var(--spm-acc)' : 'transparent', color: selected || today ? '#fff' : 'var(--spm-t3)' }}>
                  <span>{format(day, 'E', { locale: ko })}</span>
                  <strong className="mt-1 text-[16px]" style={{ fontFamily: 'var(--spm-font-display)' }}>{format(day, 'd')}</strong>
                  {dayLessons.length > 0 ? <span className="absolute bottom-1.5 h-1.5 w-1.5 rounded-full" style={{ background: selected || today ? '#fff' : 'var(--spm-acc)' }} /> : null}
                </button>
              );
            })}
          </div>
