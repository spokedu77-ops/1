  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      <header className="px-[22px] pb-5 pt-[22px] sm:px-8 lg:px-10">
        <p className="text-[12px] font-semibold uppercase tracking-[0.14em]" style={{ color: 'var(--spm-t3)' }}>equipment shop</p>
        ...
        <p className="mt-2 max-w-[720px] text-[13px] font-medium leading-6" style={{ color: 'var(--spm-t2)' }}>
          프로그램 라이브러리와 연결되는 수업 준비 영역입니다. 학생 이력 데이터와 분리된 교구 구매 흐름으로, 강사와 센터가 필요한 장비를 빠르게 장바구니에 담습니다.
        </p>
      </header>
      ...
      <BottomSheet open={orderOpen} title="주문 요청 완료" onClose={() => setOrderOpen(false)}>
        <div className="py-4 text-center">
          ...
          <div className="mt-6 grid grid-cols-2 gap-2">
            <button type="button" onClick={() => setOrderOpen(false)} className="h-11 rounded-[12px] text-[13px] font-black" style={{ background: 'var(--spm-s3)', color: 'var(--spm-t)' }}>닫기</button>
            <Link href="/spokedu-master/library" className="flex h-11 items-center justify-center rounded-[12px] text-[13px] font-black text-white" style={{ background: 'var(--spm-acc)' }}>수업안으로</Link>
          </div>
        </div>
      </BottomSheet>
    </div>
