...
const migration = [
  read('supabase/migrations/20260911173000_spokedu_master_session_integrity.sql'),
  read('supabase/migrations/20260912120000_spokedu_master_completed_edits_and_recurring_activities.sql'),
].join('\n');
const latestMigration = read('supabase/migrations/20260912120000_spokedu_master_completed_edits_and_recurring_activities.sql');
...
describe('SPOKEDU MASTER session integrity contract', () => {
  it('blocks incomplete attendance at completion in client, API, and DB', () => {
    ...
    expect(sessionsRoute).toContain('buildCompletionRosterStudentIds');
    expect(sessionsRoute).toContain('validateCompletionAttendance(rosterStudentIds, body.attendance)');
    ...
    expect(latestMigration).toContain("if v_session.status='completed' then");
    expect(latestMigration).toContain('spokedu_master_replace_session_attendance');
    expect(latestMigration).toContain("update public.spokedu_master_sessions set memo=");
  });
  ...
  it('unifies same-class overlap across create, next, recurring, and save paths', () => {
    ...
    expect(scheduleRoute).toContain("rpc('spokedu_master_materialize_schedule_rule_with_activities'");
    ...
    expect(latestMigration).toContain('for v_activity in');
