  ...
  return <BottomSheet open title={`학생 추가 · ${className}`} onClose={onClose}>
    <div className="space-y-4 pb-3">
      {mode === 'search' ? <>
        ...
        {selectedIds.length ? <div className="sticky bottom-0 rounded-2xl border border-emerald-200 bg-white p-3 shadow-lg"><p className="mb-2 text-center text-xs font-black text-emerald-700">{selectedIds.length}명 선택</p><button type="button" disabled={saving} onClick={() => void addSelected()} className="min-h-11 w-full rounded-xl bg-emerald-600 text-sm font-black text-white disabled:opacity-40">선택한 학생 추가</button></div> : null}
      ...
      {mode === 'new' ? <>
        ...
        <button type="button" disabled={!name.trim() || saving} onClick={() => void createOne()} className="min-h-11 w-full rounded-xl bg-emerald-600 text-sm font-black text-white disabled:opacity-40">등록하고 추가</button>
      </> : null}
