...
function SessionSummary({ session, workState, emptyHref }: { session: ReturnType<typeof selectNextClassSession>; workState?: MasterSessionWorkState | null; emptyHref: string }) {
  ...
  return <section className="rounded-2xl bg-slate-900 p-5 text-white">
    <p className="text-xs font-bold text-emerald-300">지금 해야 할 일</p>
    <h2 className="mt-2 text-xl font-bold">{actionLabel}</h2>
    ...
    <Link href={session ? workState?.href ?? `/spokedu-master/activity?session=${encodeURIComponent(session.id)}` : emptyHref} className="mt-5 flex min-h-12 w-full items-center justify-center gap-1 rounded-xl bg-emerald-500 px-4 text-sm font-bold text-slate-950">{actionLabel}<ChevronRight size={16} /></Link>
  </section>;
}
...
export default function ClassDetailPage() {
  ...
  return <main className="h-full overflow-y-auto bg-[var(--spm-bg)] pb-28 lg:pb-8">
    <div className="mx-auto w-full max-w-6xl px-4 py-5 sm:px-6">
      ...
      <div className="mt-5"><SessionSummary session={priorityWork?.session ?? nextSession} workState={priorityWork?.workState} emptyHref={createSessionHref} /></div>
      ...
      <div className="mt-7 flex items-center justify-between"><div><p className="text-xs font-bold text-emerald-700">학생</p><h2 className="mt-1 text-lg font-bold text-slate-900">함께하는 학생 {roster.length}명</h2></div>{roster.length ? <button type="button" onClick={() => setAddOpen(true)} className="min-h-11 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-600"><Plus size={15} className="mr-1 inline" />학생 추가</button> : null}</div>
