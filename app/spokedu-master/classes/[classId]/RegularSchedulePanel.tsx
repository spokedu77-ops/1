...
import { SPM_PRIMARY_BTN, SPM_PRIMARY_BTN_FULL, SPM_SECONDARY_BTN } from '../../lib/masterActionGrammar';
...
export function RegularSchedulePanel({ classId, sessions, onCreated }: { classId: string; sessions: MasterSessionDto[]; onCreated: () => Promise<void> }) {
  ...
  return <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-black text-slate-500">정기 수업 일정</p><h2 className="mt-1 text-lg font-black text-slate-900">{activeRules.length ? activeRules.map((rule) => `${rule.cadence === 'biweekly' ? '격주' : '매주'} ${WEEKDAYS[rule.weekday]} · ${rule.startTime}`).join(' / ') : '설정된 일정 없음'}</h2></div><button type="button" onClick={() => setOpen(true)} className={activeRules.length ? SPM_SECONDARY_BTN : SPM_PRIMARY_BTN}><Plus size={16} />정기 일정 설정</button></div>
    <p className="mt-2 text-xs font-semibold leading-5 text-slate-500">일정은 명시적으로 생성합니다. 개별 수업 변경·취소는 정기 일정에 영향을 주지 않습니다.</p>
    {activeRules.length ? <div className="mt-3 space-y-2">{activeRules.map((rule) => <div key={rule.id} className="flex min-h-11 items-center justify-between rounded-xl bg-slate-50 px-3"><span className="text-xs font-bold text-slate-600">이미 만든 미래 수업은 유지됩니다.</span><button type="button" disabled={saving} onClick={() => void endRule(rule.id)} className="min-h-11 px-2 text-xs font-black text-rose-600">정기 일정 종료</button></div>)}</div> : null}
