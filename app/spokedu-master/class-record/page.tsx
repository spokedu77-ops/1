...
function EmptyRecordState() {
  return (
    <div className="rounded-[18px] p-6 text-center" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
      ...
      <p className="mx-auto mt-2 max-w-[420px] text-[13px] font-medium leading-6" style={{ color: 'var(--spm-t2)' }}>라이브러리에서 수업안을 선택해 첫 기록을 시작하세요. 기록이 쌓이면 학생 이력, 보호자 공유, 학기말 리포트가 자동으로 이어집니다.</p>
  ...
  return (
    <article className="rounded-[16px] p-4" style={{ background: 'var(--spm-s2)', border: '1px solid var(--spm-br2)' }}>
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="shrink-0 rounded-full px-3 py-1.5 text-[11px] font-black" style={{ background: record.kakaoSent ? 'rgba(16,185,129,0.13)' : 'rgba(99,102,241,0.13)', color: record.kakaoSent ? 'var(--spm-grn)' : 'var(--spm-acc)' }}>{record.kakaoSent ? '발송 완료' : '기록 완료'}</span>
      </div>
      ...
      <Link href="/spokedu-master/report" className="mt-4 flex h-11 items-center justify-center rounded-[12px] text-[13px] font-black" style={{ background: 'var(--spm-s3)', color: 'var(--spm-t)' }}>리포트에서 보기</Link>
    </article>
  );
}
  ...
  return (
    <div className="h-full overflow-y-auto pb-7" style={{ background: 'var(--spm-bg)' }}>
      <header className="px-[22px] pb-5 pt-[22px] sm:px-8 lg:px-10">
        ...
        <p className="mt-2 max-w-[680px] text-[13px] font-medium leading-6" style={{ color: 'var(--spm-t2)' }}>완료된 수업 기록과 미완료 보완 항목을 최신순으로 확인합니다. v4.2 기준으로 기록 탭은 수업의 아카이브이자 다시 시작하는 허브입니다.</p>
      </header>
