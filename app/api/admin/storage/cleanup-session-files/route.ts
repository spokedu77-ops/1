...
import { alignCenterDocumentNamesWithUrls, type FeedbackFields } from '@/app/lib/feedbackValidation';

/** 버킷 전체 스캔·삭제는 수 분 걸릴 수 있음 (플랫폼 기본 제한 회피) */
export const maxDuration = 300;

const TARGET_BUCKET = CENTER_SESSION_FILES_BUCKET;
  ...
  try {
    ...
    while (true) {
      const { data: sessions, error } = await supabase
        .from('sessions')
        .select('id, file_url, feedback_fields')
      ...
      for (const row of rows) {
        const current = Array.isArray(row.file_url)
          ? row.file_url.filter((v): v is string => typeof v === 'string')
          : [];
        ...
        const ffRaw = row.feedback_fields;
        const feedbackFields: FeedbackFields =
          ffRaw && typeof ffRaw === 'object' && !Array.isArray(ffRaw) ? (ffRaw as FeedbackFields) : {};

        const nextFeedback: FeedbackFields = {
          ...feedbackFields,
          center_document_names: alignCenterDocumentNamesWithUrls(next, feedbackFields.center_document_names),
        };

        const { error: updateErr } = await supabase
          .from('sessions')
          .update({ file_url: next, feedback_fields: nextFeedback })
          .eq('id', row.id);
    ...
    devLogger.error('[cleanup-session-files]', err);
    return NextResponse.json(
      { error: '센터 첨부(session-files) 정리 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
