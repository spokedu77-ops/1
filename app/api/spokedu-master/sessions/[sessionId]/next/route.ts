...
export async function POST(request: Request, context: { params: Promise<{ sessionId: string }> }) {
  ...
  let { data, error: loadError } = await supabase.from('spokedu_master_sessions').select(SESSION_SELECT).eq('id', nextId).eq('owner_id', access.userId).single();
  if (loadError && String(loadError.message ?? '').includes('roster_locked_at')) {
