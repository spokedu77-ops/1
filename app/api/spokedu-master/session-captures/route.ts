...
const SELECT = `id,session_id,legacy_id,class_date,lesson_title,class_id,program_id,program_title,record_type,memo,application_idea,parent_note_snapshot,created_at,updated_at,deleted_at,spokedu_master_class_record_students(id,owner_id,record_id,student_id,student_legacy_id,student_name_snapshot,attendance,focused,skills,memo,observation_score,created_at,updated_at)`;
...
export async function GET(request: Request) {
  ...
  let query = getServiceSupabase().from('spokedu_master_class_records').select(SELECT)
  ...
  const captures = ((data ?? []) as MasterClassRecordRow[]).map(toClassRecordDto)
    .filter((capture) => !studentId || capture.students.some((student) => student.studentId === studentId));
