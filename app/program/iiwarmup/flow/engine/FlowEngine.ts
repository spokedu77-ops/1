...
export class FlowEngine {
  ...
  private createSpaceBackground(): void {
    ...
    if (!path || typeof path !== 'string' || path.trim() === '' || typeof window === 'undefined') {
    ...
    const url = isFullUrl
      ? trimmed
      : (base
          ? `${base}/storage/v1/object/public/iiwarmup-files/${trimmed.split('/').map((seg) => encodeURIComponent(seg)).join('/')}`
          : '');
    if (!url) {
      ...
      .then((res) => {
        ...
        loader.load(
          ...
          (tex) => {
            ...
            const fileName = this.panoStoragePath ? this.panoStoragePath.split('/').pop() ?? '' : '';
            this.setPanoStatus(`PANO: ok (mesh:${inScene ? 1 : 0} map:${hasMap ? 1 : 0} srgb:${srgb}) ${fileName}`);
          },
          undefined,
          () => {
