...
export function StageAQuad({ activeColor, imageUrl, frame }: StageAQuadProps) {
  ...
  return (
    <div className="grid h-full min-h-[200px] w-full grid-cols-2 grid-rows-[1fr_1fr] gap-0">
      {PAD_POSITIONS.map((row, ri) =>
        row.map((color, ci) => {
          ...
          return (
            <div
              key={`${ri}-${ci}`}
              className="relative flex min-h-[100px] items-center justify-center overflow-hidden"
