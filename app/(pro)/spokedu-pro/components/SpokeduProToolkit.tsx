...
export default function SpokeduProToolkit({
  ...
  const panel = (
    <div className="space-y-4">
      ...
      <div className="mt-2 space-y-3 border-t border-slate-700 pt-5">
  ...
  if (variant === 'embedded') {
    return (
      <div className="spokedu-pro w-full overflow-y-auto custom-scroll">
        {panel}
      </div>
    );
  }
  ...
  const stack = (
    <div className="spokedu-pro pointer-events-none fixed inset-0 z-[220] overflow-visible">
      <div className="pointer-events-auto flex flex-col items-end gap-4 fixed right-3 bottom-[max(0.75rem,calc(5.35rem+env(safe-area-inset-bottom)))] md:bottom-6 md:right-6">
        <div
          className={`tool-panel sp-pro-glass-panel rounded-2xl shadow-2xl shadow-black/40 p-6 w-[min(100vw-1.5rem,340px)] md:w-[340px] overflow-y-auto max-h-[min(70vh,520px)] md:max-h-[85vh] custom-scroll ${open ? 'show' : ''}`}
        >
          <div className="flex justify-between items-center mb-6 border-b border-slate-600/50 pb-3 sticky top-0 z-10 -mx-2 -mt-1 rounded-t-xl bg-slate-950/75 px-2 pt-1 backdrop-blur-sm">
