...
export function assertPadGridMatch(grid: PADColor[][]): void {
  ...
  if (grid[0][0] !== PAD_GRID.TOP_LEFT || grid[0][1] !== PAD_GRID.TOP_RIGHT) {
    throw new Error(`assertPadGridMatch: top row mismatch. expected [red, yellow], got [${grid[0][0]}, ${grid[0][1]}]`);
  }
  if (grid[1][0] !== PAD_GRID.BOTTOM_LEFT || grid[1][1] !== PAD_GRID.BOTTOM_RIGHT) {
    throw new Error(`assertPadGridMatch: bottom row mismatch. expected [green, blue], got [${grid[1][0]}, ${grid[1][1]}]`);
  }
}
