...
const COLOR_CUSTOM: SpomoveSequentialMemoryGuideSeed = {
  ...
  movementGuide: {
    movement: { baseMovement: 'stepHold', limbRule: 'free' },
    objective: '교사가 지정한 커스텀 색 순서를 제시받아 기억한 뒤, 같은 순서로 패드를 재현합니다.',
    teachingPoints: [
      '교사가 정한 순서가 SSOT이므로, 화면 제시와 교사 의가 같은지 먼저 맞추세요.',
      '긴 서열일수록 덩어리(앞·중간·뒤)로 나눠 기억하게 하세요.',
      '재현이 끝나면 교사가 지정표와 대조해 확인합니다. 화면만 보고 끝내지 마세요.',
    ],
    instruction:
      '교사가 정한 색 순서가 화면에 제시되는 동안 끝까지 봅니다.\n제시가 끝나면 같은 순서로 패드를 밟아 재현합니다.\n교사가 지정 순서와 맞춰 확인한 뒤 다음으로 갑니다.',
    coachScript: '내가 정한 순서! 보고 기억하고 그대로!',
    focusTags: tags('sequenceMemory', 'attentionShift', 'individual'),
    easier: '슬롯 수를 줄이거나 반복 색을 늘려 기억 부담을 낮춥니다.',
    harder: '슬롯을 늘리고, 재현 전 되뇌기 없이 바로 수행하게 합니다.',
    successCriteria: '교사 지정 순서를 빠짐없이 재현하고 교사 확인을 받습니다.',
    commonMistake: '익숙한 기본 3색 패턴으로 바꿔 밟거나, 교사 확인 없이 다음으로 갑니다.',
  },
};
